...
print(paste("🎯 Complementação FUNDEB (100%):", formatar_numero(complementacao_fundeb)))
```
...
# ------------------------------------------------------------------------------
# VARIÁVEL MANUAL - Saldo inicial da dívida (ajustar anualmente)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Filtro: Item Informação = 5 (Receita Líquida), Origem = 21 ou 81
...
# ------------------------------------------------------------------------------
# 2. DESPESAS DE CAPITAL (Liquidadas + Inscritas em RP Não Processados)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 3. SUBCONTA DA DÍVIDA (Saldo atual)
# ------------------------------------------------------------------------------
...
# ==============================================================================
# CONSOLIDAÇÃO MASTER - FONTE ÚNICA DA VERDADE
# ==============================================================================

dados_consolidados <- consolidar_criterios(save_global = FALSE)

message(sprintf("✅ Consolidação: %d linhas de %d relatórios", 
...
# ------------------------------------------------------------------------------
# RGF A01 - PESSOAL (JÁ CRIADO ANTES COM aplicar_criterios INLINE)
# ------------------------------------------------------------------------------

# Os objetos pessoal e pessoal_rp já foram criados pelos chunks anteriores
# Apenas verificamos se existem

if (exists("pessoal") && exists("pessoal_rp")) {
  message(sprintf("✅ RGF A01 - Pessoal: %d linhas | RP: %d linhas", 
                  nrow(pessoal), nrow(pessoal_rp)))
} else {
  warning("⚠️ RGF A01 - Objetos pessoal/pessoal_rp não encontrados")
  pessoal <- tibble()
  pessoal_rp <- tibble()
}
...
message(sprintf("✅ RGF A02 - DCL: %d linhas", nrow(dcl_completo)))
...
if (nrow(garantias) == 0) {
  warning("⚠️ RGF A03 - Garantias não encontrado")
  warning("   Verificar se aplicar_criterios foi extraído do datatable()")
} else {
  message(sprintf("✅ RGF A03 - Garantias: %d linhas", nrow(garantias)))
}
...
message(sprintf("✅ RGF A04 - Operações: %d linhas", nrow(operacoes_credito)))
...
if (nrow(disponibilidades) == 0) {
  warning("⚠️ RGF A05 - Disponibilidades não encontrado")
  warning("   Verificar se versão simplificada foi criada")
} else {
  message(sprintf("✅ RGF A05 - Disponibilidades: %d linhas", nrow(disponibilidades)))
}
...
# 🔧 USAR anexo_nome EM VEZ DE detalhe
...
message(sprintf("✅ RREO A01 - Balanço: %d linhas (receitas: %d | despesas: %d)", 
...
message(sprintf("✅ RREO A03 - RCL: %d linhas", nrow(rcl)))
...
message(sprintf("✅ RREO A04 - Previdência: %d linhas", nrow(previdencia)))
...
datatable(dados_despesa %>% group_by( refinanciamento) %>% filter(item_informacao_codigo == 23) %>% summarise(saldo_r_item_informacao = sum(saldo_r_item_informacao)) %>% adorn_totals(), caption = "RREO Anexo 02: Função" )%>% formatRound("saldo_r_item_informacao", 2, mark = ".", dec.mark = "," )%>% 
  DT::formatStyle(columns = colnames(.$x$data), fontSize = '100%')
...
# ==============================================================================
# RREO ANEXO 6 - RESULTADO PRIMÁRIO
# ==============================================================================
...
resultado_primario <- tibble(
  componente = c(
    "RECEITA PRIMÁRIA CORRENTE (IV)",
    "RECEITA PRIMÁRIA DE CAPITAL (XI)",
    "RECEITA PRIMÁRIA TOTAL (XII)",
    "DESPESA PRIMÁRIA CORRENTE (XV)",
    "DESPESA PRIMÁRIA DE CAPITAL (XXI)",
    "(+) Subsídios CESEF",
  ...
  mutate(
    valor = case_when(
      componente == "RECEITA PRIMÁRIA TOTAL (XII)" ~ valor[1] + valor[2],
      componente == "DESPESA PRIMÁRIA TOTAL (XXIII)" ~ valor[4] + valor[5] + valor[6],
      componente == "RESULTADO PRIMÁRIO (XII - XXIII)" ~ (valor[1] + valor[2]) - (valor[4] + valor[5] + valor[6]),
      TRUE ~ valor
    )
  )
...
# Saúde - usando print() e datatable() em vez de cat()

# Exibir mínimo saúde como tabela
datatable(
  data.frame(
    Indicador = "Mínimo Constitucional Saúde (15% RCL)",
...
# Calcular valor do mínimo para comparação
...
datatable(
  data.frame(
    Indicador = c("Total Despesas Saúde (R$ mil)", "Mínimo Constitucional", "Cumprimento (%)"),
    Valor = c(
  ...
  options = list(dom = 't'),
  rownames = FALSE,
  caption = "💰 Cumprimento do Mínimo Constitucional - Saúde"
)
  ...
  aplicar_criterios(
    df = dados_despesa %>% 
      filter(
        ...
        mes_lancamento == mes_filtro,
        item_informacao_codigo %in% c(13, 25)  # Dotação e Liquidada
      ),
    criterios = criterios_rreo_T02_despesas_T02,
...
cat( "\n💰 **Tabela 03:** ", formatar_numero((dados_despesa %>% 
  filter(resultado_eof_codigo == 6, item_informacao_codigo == 23) %>% summarise(saldo_r_item_informacao = sum(saldo_r_item_informacao, na.rm =  TRUE)))$saldo_r_item_informacao), "\n") 
...
datatable(
  aplicar_criterios(
    ...
    summarise(valor = sum(valor, na.rm = TRUE)) %>% 
    pivot_wider(names_from = "orgao_uge_tipo_administracao_nome", values_from = "valor") %>%
    # Reordenar colunas conforme relatório oficial
    select(
      `ADMINISTRACAO DIRETA`,
      AUTARQUIA,
      FUNDACAO,
...
# 
# # No seu código, após definir mes_pasta:
# df_arquivos_tg <- mapear_arquivos_tg(mes_pasta)
# 
# # Visualizar
# datatable(df_arquivos_tg)
...
# A aba "Comparativo CESEF" recebe dados da outra área (RTN)

arquivo_rp <- str_c(mes_pasta, "rp_claude.xlsm")

# Resultado Primário - célula E53
valor_res_prim <- tryCatch({
  if (file.exists(arquivo_rp)) {
    ...
    # Fallback: usar cálculo do sistema (sem ajuste metodológico)
    res <- resultado_primario %>%
      filter(componente == "RESULTADO PRIMÁRIO (XII - XXIII)") %>%
      pull(valor)
    if (length(res) == 0) NA_real_ else as.numeric(res[1])
  }
}, error = function(e) NA_real_)

# Resultado Nominal - célula C60
...
valor_mde_minimo <- tryCatch({
  if (exists("rli_atual")) {
    ...
    texto <- resumo_aplicacao %>% filter(grepl("18%|Base de Cálculo|RLI", Item)) %>% pull(Valor)
    if (length(texto) == 0) return(NA_real_)
    as.numeric(gsub("[^0-9.-]", "", texto[1]))
  }
}, error = function(e) NA_real_)
...
valor_fundeb <- tryCatch({
  if (exists("complementacao_fundeb")) {
    ...
    texto <- resumo_aplicacao %>% filter(grepl("Complementação FUNDEB|FUNDEB", Item)) %>% pull(Valor)
    if (length(texto) == 0) return(NA_real_)
    as.numeric(gsub("[^0-9.-]", "", texto[1]))
  }
}, error = function(e) NA_real_)
...
# Despesas Pagas (codigo 28) - APENAS exercicio, não RAP
  ...
  dados_rp_anexo_07 %>%
    filter(mes_lancamento == mes_filtro,
           item_informacao_nome %in% c("RP Processados Inscritos em 31 de Dezembro do Ano Anterior",
                                       "RP Processados Inscritos em Exercícios Anteriores")) %>%
    summarise(v = sum(saldo_r_item_informacao, na.rm = TRUE)) %>%
    pull(v)
}, error = function(e) NA_real_)
  ...
  dados_rp_anexo_07 %>%
    filter(mes_lancamento == mes_filtro,
           item_informacao_nome %in% c("RPNP Inscritos em 31 de Dezembro do Ano Anterior",
                                       "RPNP Inscritos em Exercícios Anteriores")) %>%
    summarise(v = sum(saldo_r_item_informacao, na.rm = TRUE)) %>%
    pull(v)
}, error = function(e) NA_real_)
