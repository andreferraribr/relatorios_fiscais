...
# Carregar bibliotecas de paralelização
suppressPackageStartupMessages({
  library(furrr)      # Para processamento paralelo com purrr
  library(future)     # Backend de paralelização
  library(data.table) # Para operações ultrarrápidas em dados grandes
})

# Detectar recursos disponíveis
cores_disponiveis <- parallel::detectCores()

# Obter memória total (compatível com Windows)
...
cat("\n")
cat("╔══════════════════════════════════════════════════════════════╗\n")
cat("║        ⚡ OTIMIZAÇÃO DE PERFORMANCE - SISTEMA ATIVADO        ║\n")
cat("╠══════════════════════════════════════════════════════════════╣\n")
cat(sprintf("║ 🖥️  CPU: %d cores disponíveis%s║\n", 
    cores_disponiveis, 
    strrep(" ", 44 - nchar(as.character(cores_disponiveis)))))
cat(sprintf("║ 💾 RAM: %.1f GB%s║\n", 
    memoria_info, 
    strrep(" ", 48 - nchar(sprintf("%.1f GB", memoria_info)))))

# Configurar paralelização (usar até 10 cores, deixar 2 para o sistema)
cores_usar <- min(10, max(1, cores_disponiveis - 2))
cat(sprintf("║ ⚙️  Cores para processamento paralelo: %d%s║\n", 
    cores_usar,
    strrep(" ", 29 - nchar(as.character(cores_usar)))))

# Configurar backend de paralelização
plan(multisession, workers = cores_usar)

# Desabilitar paralelização automática do data.table para evitar conflitos
setDTthreads(1)

cat("╠══════════════════════════════════════════════════════════════╣\n")
cat("║ ✅ Paralelização configurada e ativa                        ║\n")
cat("║ ✅ Otimizações de memória ativadas                          ║\n")
cat("║ ✅ Backend: future::multisession                            ║\n")
cat("╚══════════════════════════════════════════════════════════════╝\n")
cat("\n")

# ============================================================================
# FUNÇÕES OTIMIZADAS PARA IMPORTAÇÃO PARALELA
# ============================================================================

# ============================================================================
# 🔧 CORREÇÃO: Função importar_excel_paralelo ATUALIZADA
# ============================================================================
# SUBSTITUA a função antiga no chunk {r setup} por esta versão
...
importar_excel_paralelo <- function(arquivos, skip = 5, .progress = TRUE) {
  ...
  cat(sprintf("📥 Importando %d arquivos em paralelo...\n", length(arquivos)))
  ...
  resultado <- future_map_dfr(
    ...
    function(arq) {
      ...
      # ============================================================================
      # Padrões de nomes de colunas que devem ser numéricas
      numeric_patterns <- c(
      ...
      # Encontrar colunas que correspondem aos padrões
      cols_numericas <- names(df)[str_detect(names(df), 
                                              paste(numeric_patterns, collapse = "|"))]
      
      # Converter para numérico (com suppressWarnings para NAs esperados)
  ...
  cat(sprintf("✅ Importação concluída: %s registros\n", 
  ...
  if (length(cols_convertidas) > 0) {
    cat(sprintf("   📊 Colunas numéricas: %d\n", length(cols_convertidas)))
  }
  
  return(resultado)
}

cat("✅ Função importar_excel_paralelo() ATUALIZADA carregada!\n\n")
...
cat("📥 Importando dados de receita em paralelo...\n")
...
dados_receita <- importar_excel_paralelo(
  ...
  # Filtrar e transformar uma única vez
...
cat(sprintf("✅ Dados de receita carregados: %s registros\n", 
            format(nrow(dados_receita), big.mark = ".", decimal.mark = ",")))
cat(sprintf("   • Memória utilizada: %.1f MB\n", 
...
cat(sprintf("📥 Importando %d arquivos de despesa em paralelo...\n", length(arq_despesa)))
...
cat(sprintf("✅ Dados de despesa carregados: %s registros\n", 
...
cat("\n📊 Resumo por tipo_modalidade:\n")
print(dados_despesa %>% count(tipo_modalidade, sort = TRUE))

cat("\n📊 Resumo por refinanciamento:\n")
...
# ============================================================================
# ⚡ VERSÃO OTIMIZADA - IMPORTAÇÃO PARALELA MANTENDO ESTRUTURAS SEPARADAS
# ============================================================================
...
cat(sprintf("📥 Importando %d arquivos de RP em paralelo...\n", length(arquivos_rp)))

# Importar todos simultaneamente, cada um mantém sua estrutura própria
...
# Extrair cada arquivo para sua variável específica
# Anexo 07 precisa de transformações adicionais
...
# Outros anexos mantêm estrutura básica
...
cat(sprintf("   • Memória total: %.1f MB\n", memoria_total))
cat(sprintf("   • Tempo de importação: %.1f segundos\n", 
            as.numeric(difftime(tempo_fim, tempo_inicio, units = "secs"))))
```

## 🏦 Dados RREO Anexo 06 - Resultado Primário
...
# ============================================================================
# ⚡ VERSÃO OTIMIZADA - IMPORTAÇÃO PARALELA DE 4 ARQUIVOS
# ============================================================================
...
cat(sprintf("📥 Importando %d arquivos do Anexo 06 em paralelo...\n", length(arquivos_a06)))
...
# Extrair para variáveis específicas
...
cat("\n✅ Dados do Anexo 06 carregados:\n")
cat(sprintf("   • NDD: %s registros\n", 
            format(nrow(dados_ndd), big.mark = ".", decimal.mark = ",")))
cat(sprintf("   • Categoria/Grupo: %s registros\n", 
            format(nrow(dados_categoria_grupo), big.mark = ".", decimal.mark = ",")))
cat(sprintf("   • Juros: %s registros\n", 
            format(nrow(dados_juros), big.mark = ".", decimal.mark = ",")))
cat(sprintf("   • Disponibilidades: %s registros\n", 
...
# Verificar que alias foi criado e é numérico
cat("\n✅ Alias 'saldo_r_item_informacao' criado:\n")
...
cat("\n✅ Importação RCL concluída com sucesso!\n")
```
...
# Função auxiliar para processar arquivo (mantida para compatibilidade)
...
#' Aplica critérios de filtro e agrupamento a um dataframe
#'
#' @param df Dataframe de entrada com dados fiscais
#' @param criterios Lista nomeada de critérios (cada item com campo 'criterio')
#' @param group_vars Vetor de variáveis para agrupamento (opcional)
#' @param save_global Se TRUE, salva resultado no ambiente global com nome automático (padrão: FALSE)
#' @param validate_criteria Se TRUE, valida critérios antes de executar (padrão: TRUE)
#'
...
aplicar_criterios <- function(df, 
  ...
  # ============================================================================
  # 1. DETECÇÃO AUTOMÁTICA DA MÉTRICA
  # ============================================================================
  metrica <- names(df)[grepl("^(saldo|movim)", names(df), ignore.case = TRUE)][1]
  
  if (is.na(metrica) || length(metrica) == 0) {
    stop("Não foi possível detectar coluna de métrica (saldo_* ou movim_*)")
  }
  ...
  # Capturar nome da variável criterios
  criterios_nome <- deparse(substitute(criterios))
  
  # ============================================================================
  # 3. DEFINIR VARIÁVEIS DE AGRUPAMENTO
  # ============================================================================
  if (is.null(group_vars)) {
    group_vars <- c("mes_lancamento", "data_lancamento")
  } else {
    # Garantir que mes_lancamento e data_lancamento estejam incluídos
  ...
  # ============================================================================
  # 4. EXTRAIR METADADOS DOS CRITÉRIOS
  # ============================================================================
  ...
  if (validate_criteria) {
    for (cat_nome in names(criterios)) {
      ...
      tryCatch({
        ...
        warning(sprintf("⚠️ Critério inválido em categoria '%s': %s\n   Expressão: %s", 
  ...
  # ============================================================================
  # 6. APLICAR CRITÉRIOS (COM RLANG - SEGURO)
  # ============================================================================
  resultado <- map_df(names(criterios), function(categoria) {
    ...
    # ========================================================================
    # 🔧 CORREÇÃO PRINCIPAL: Usar rlang em vez de eval(parse())
    # ========================================================================
    condicao_expr <- tryCatch({
      # Parsear expressão de forma segura
      ...
      stop(sprintf(
        "❌ Erro ao avaliar critério em categoria '%s':\n   Expressão: %s\n   Erro: %s", 
        categoria, crit$criterio, e$message
      ))
    })
    
    # Verificar se retornou vetor lógico
    if (!is.logical(condicao_expr)) {
      stop(sprintf(
        "❌ Critério não retornou vetor lógico em categoria '%s':\n   Expressão: %s\n   Tipo retornado: %s",
  ...
  criterios_nome_completo = criterios_nome,  # ← ADICIONAR ISTO
  relatorio = relatorio,
  ...
  filtro = crit$criterio
    ...
    # ========================================================================
    # 🔧 CORREÇÃO: Usar str_extract em vez de separate(sep = 2)
    # ========================================================================
    resultado_final <- resultado_filtrado %>%
      mutate(
        # Extrair ordem (1 ou 2 dígitos no início)
        ordem = str_extract(categoria, "^\\d{1,2}"),
        # Extrair nome (tudo após os dígitos e espaços)
  ...
  if (save_global) {
    ...
    message("✅ Resultado salvo em variável global: ", novo_nome)
  }
...
consolidar_criterios <- function(save_global = FALSE) {
  ...
  if (length(df_criterios) == 0) {
    warning("⚠️ Nenhuma dataframe com prefixo 'df_criterios' foi encontrada no ambiente global")
    return(data.frame())
  }
  
  message(sprintf("📊 Encontradas %d dataframes para consolidar", length(df_criterios)))
  ...
  tabelas_padronizadas <- map(tabelas, function(tabela) {
    ...
    # Converter colunas problemáticas para character
  ...
  message(sprintf("✅ Consolidadas %d linhas de dados", nrow(resultado_completo)))
  
  # ============================================================================
  # 4. 🔧 CORREÇÃO: IDENTIFICAR TODAS AS COLUNAS PARA AGRUPAMENTO
  # ============================================================================
  # Colunas padrão que sempre existem
  ...
  # 🔧 NOVO: Identificar colunas EXTRAS que vieram de group_vars
  ...
  # Colunas para agrupamento = padrão + extras
  ...
  message(sprintf("📋 Agrupando por %d colunas (incluindo variáveis customizadas)", 
                  length(colunas_agrupamento)))
  
  # ============================================================================
  # 5. AGRUPAR E SUMARIZAR (PRESERVANDO TODAS AS DIMENSÕES)
  # ============================================================================
  ...
  # ============================================================================
  # 7. REORDENAR COLUNAS (PADRÃO + EXTRAS + VALOR)
  # ============================================================================
  colunas_ordem <- c(
  ...
  if (save_global) {
    ...
    message("✅ Dados consolidados salvos em: df_criterios_consolidado")
  }
...
# 📋 RGF Anexo 01 - Despesa com Pessoal {#sec-rgf-a01}
...
# =============================================================================
# FICHA TÉCNICA DO ANEXO
# =============================================================================

ficha_tecnica <- tibble::tribble(
  ~Campo,                 ~Informação,
  "Relatório",            "RGF - Relatório de Gestão Fiscal",
  "Anexo",                "Anexo 01",
  "Nome",                 "Demonstrativo da Despesa com Pessoal",
  "Base Legal",           "LRF Art. 55, inciso I, alínea 'a'",
  "Periodicidade",        "Quadrimestral",
  "Grau de Dificuldade",  "⭐⭐⭐⭐ Difícil",
  "Última Atualização",   format(Sys.Date(), "%d/%m/%Y")
)

knitr::kable(ficha_tecnica, col.names = c("Campo", "Informação"))
```
...
arquivos <- tibble::tribble(
  ~Arquivo,                    ~Descrição,                                    ~Filtros_TG,
  "rgf_a01_jan.xlsx",          "Despesa pessoal - Janeiro",                   "Mês = 01",
  "rgf_a01_fev.xlsx",          "Despesa pessoal - Fevereiro",                 "Mês = 02",
  "rgf_a01_mar.xlsx",          "Despesa pessoal - Março",                     "Mês = 03",
  "rgf_a01_abr.xlsx",          "Despesa pessoal - Abril",                     "Mês = 04",
  "rgf_a01_mai.xlsx",          "Despesa pessoal - Maio",                      "Mês = 05",
  "rgf_a01_jun.xlsx",          "Despesa pessoal - Junho",                     "Mês = 06",
  "rgf_a01_jul.xlsx",          "Despesa pessoal - Julho",                     "Mês = 07",
  "rgf_a01_ago.xlsx",          "Despesa pessoal - Agosto",                    "Mês = 08",
  "rgf_a01_set.xlsx",          "Despesa pessoal - Setembro",                  "Mês = 09",
  "rgf_a01_out.xlsx",          "Despesa pessoal - Outubro",                   "Mês = 10",
  "rgf_a01_nov.xlsx",          "Despesa pessoal - Novembro (se aplicável)",   "Mês = 11",
  "rgf_a01_dez.xlsx",          "Despesa pessoal - Dezembro (se aplicável)",   "Mês = 12"
)

knitr::kable(arquivos, col.names = c("Arquivo", "Descrição", "Filtros no TG"))
```
...
#   Critério  = Usado nos filtros/critérios
#   Apresentação = Usado na exibição final

atributos <- tibble::tribble(
  ~Atributo,                          ~Tipo,       ~Uso,                         ~Descrição,
  "grupo_despesa_codigo_grupo",       "character", "TG / Critério",              "Código do grupo de despesa (1=Pessoal)",
  "elemento_despesa_codigo",          "character", "TG / Critério",              "Código do elemento de despesa",
  "natureza_despesa_detalhada_codigo","character", "TG / Critério",              "Código detalhado da natureza de despesa",
  "natureza_despesa_detalhada_nome",  "character", "TG / Critério",              "Nome da natureza de despesa (para detecção de termos)",
  "modalidade_aplicacao_codigo",      "character", "TG / Critério",              "Código da modalidade de aplicação",
  "fonte_recursos_codigo",            "character", "TG / Critério",              "Código da fonte de recursos",
  "unidade_orcamentaria_codigo",      "character", "TG / Critério",              "Código da UO (para AP, RR, DF)",
  "plano_orcamentario_codigo_po",     "character", "TG / Critério",              "Código do Plano Orçamentário",
  "acao_governo_codigo",              "character", "TG / Critério",              "Código da ação de governo",
  "uo_poder",                         "character", "TG / Critério",              "Identificador do Poder (0=Executivo)",
  "uo_orgao_maximo_codigo",           "character", "TG / Critério",              "Código do órgão máximo",
  "item_informacao_codigo",           "character", "TG / Critério",              "Código do item (25=Liquidado, 27=RPNP)",
  "mes_lancamento",                   "numeric",   "TG / R",                     "Mês do lançamento (YYYYMM)",
  "movim_liquido_r_item_informacao",  "numeric",   "TG / R",                     "Valor do movimento líquido",
  "data_lancamento",                  "Date",      "R",                          "Data calculada do lançamento",
  "categoria_pessoal",                "character", "R / Critério / Apresentação","Categoria: executivo, AP, RR, DF, demais"
)

knitr::kable(atributos, col.names = c("Atributo", "Tipo", "Uso", "Descrição"))
```

## Vínculos com Outros Anexos
...
vinculos <- tibble::tribble(
  ~Anexo_Relacionado,   ~Tipo_Vínculo,    ~Descrição,
  "RREO Anexo 03",      "Referência",     "RCL utilizada para cálculo dos limites",
  "RGF Anexo 06",       "Consolidação",   "Valores consolidados no demonstrativo simplificado"
)

knitr::kable(vinculos, col.names = c("Anexo Relacionado", "Tipo de Vínculo", "Descrição"))
```
...
estrutura <- tibble::tribble(
  ~Ordem, ~Item,                                           ~Grupo,
  "11",   "Vencimentos, Vantagens e Outras Desp. Variáveis", "I - Despesa Bruta",
"12",   "Obrigações Patronais",                          "I - Despesa Bruta",
  "13",   "Aposentadorias, Reserva e Reformas",             "I - Despesa Bruta",
  "14",   "Pensões",                                        "I - Despesa Bruta",
  "15",   "Outras Despesas com Inativos",                   "I - Despesa Bruta",
  "16",   "Outras Despesas de Pessoal - Terceirização",     "I - Despesa Bruta",
  "17",   "Despesa com Pessoal não Executada Orçam.",       "I - Despesa Bruta",
  "21",   "Indenizações por Demissão e Incentivos",         "II - Despesas Não Computadas",
  "22",   "Decorrentes de Decisão Judicial",                "II - Despesas Não Computadas",
  "23",   "Despesas de Exercícios Anteriores",              "II - Despesas Não Computadas",
  "24",   "Inativos e Pensionistas com Recursos Vinculados","II - Despesas Não Computadas"
)

knitr::kable(estrutura, col.names = c("Ordem", "Item", "Grupo"))
```

## Categorização por Região/Poder

```{r categorizar-rgf-a01}
#| code-fold: true
#| code-summary: "Ver código de categorização"

# =============================================================================
# CATEGORIZAÇÃO DOS DADOS POR REGIÃO/PODER
# =============================================================================
# O demonstrativo é segregado por:
#   - Poder Executivo Federal
#   - Ex-Territórios (AP, RR)
#   - Distrito Federal (DF)
# =============================================================================
...
rgf_a01 <- rgf_a01 %>%
  mutate(
    categoria_pessoal = case_when(
      ...
      # -------------------------------------------------------------------------
      # Naturezas de despesa que são ressalvadas do cálculo
      ...
      # AMAPÁ (AP)
      # -------------------------------------------------------------------------
      # UO 73113 + PO 0004 + Ações específicas
      ...
      # RORAIMA (RR)
      # -------------------------------------------------------------------------
      # UO 73113 + PO 0003 + Ações específicas
      unidade_orcamentaria_codigo == "73113" & 
      ...
      # Exceto órgãos 34000 (MPU) e 59000 (não identificado)
      uo_poder == "0" &
...
```{r criterios-rgf-a01}
#| code-fold: true
#| code-summary: "Ver critérios de classificação"

# =============================================================================
# CRITÉRIOS RGF ANEXO 01 - DEFINIÇÃO DOS FILTROS
# =============================================================================
# Numeração compatível com a estrutura do demonstrativo:
#   - Grupo I (1x): Componentes da Despesa Bruta
#   - Grupo II (2x): Despesas Não Computadas
# =============================================================================
...
criterios_rgf_A01_pessoal_rgf <- list(
  ...
  #   - Elementos 01, 03, 34 (aposentadoria, pensão, terceirização)
  #   - Obrigações patronais
  #   - Benefícios previdenciários
  #   - Termos de inativos
  #   - Despesas com inativos específicas
  ...
  `11  Vencimentos, Vantagens e Outras Despesas Variáveis` = list(
    ...
    # EXCLUSÃO: Benefícios Previdenciários
    !(
      (substr(natureza_despesa_detalhada_codigo, 1, 1) == '3' &
       grupo_despesa_codigo_grupo == '1' &
       elemento_despesa_codigo == '92' &
  ...
  # ---------------------------------------------------------------------------
  # Elementos 13 (Obrigações Patronais) e 07 (Contrib. RPPS)
  # Mais naturezas específicas de contribuições patronais
  ...
  # ---------------------------------------------------------------------------
  # Elemento 01 (Aposentadorias/Reformas/Pensões RPPS)
  # Mais naturezas específicas de aposentadorias
  ...
  # ---------------------------------------------------------------------------
  # Elemento 03 (Pensões)
  # Mais naturezas específicas de pensões
  ...
  # ---------------------------------------------------------------------------
  # Elementos específicos + detecção de termos de inativos
  # Excluindo naturezas já classificadas em outras categorias
  ...
  # ---------------------------------------------------------------------------
  # Elemento 34 (Outras Desp. Pessoal - Terceirização)
  # Grupos 1 e 3
  ...
  # ===========================================================================
  # GRUPO II - DESPESAS NÃO COMPUTADAS
  # ===========================================================================
  ...
  # ---------------------------------------------------------------------------
  # Elemento 91 (Sentenças Judiciais)
  # Excluindo fontes vinculadas com termos de inativos
  ...
  #   - Elementos de aposentadoria/pensão
...
cat("✅ Critérios RGF Anexo 01 definidos\n")
...
#| code-fold: true
#| code-summary: "Ver código de aplicação dos critérios"

# =============================================================================
# APLICAÇÃO DOS CRITÉRIOS DE DESPESA COM PESSOAL
# =============================================================================
# Item de Informação:
#   - 25 = Despesa Liquidada
#   - 27 = Restos a Pagar Não Processados (RPNP)
# =============================================================================
...
cat(sprintf("✅ Despesa liquidada processada: %d registros\n", nrow(pessoal)))

# -----------------------------------------------------------------------------
# 2. Restos a Pagar Não Processados (item 27)
# -----------------------------------------------------------------------------
...
cat(sprintf("✅ RPNP processado: %d registros\n", nrow(pessoal_rp)))

# -----------------------------------------------------------------------------
# 3. Categorias para exibição
# -----------------------------------------------------------------------------
...
## Função para Geração das Tabelas

```{r funcao-tabela-rgf-a01}
#| code-fold: true
#| code-summary: "Ver função de geração de tabelas"

# =============================================================================
# FUNÇÃO: gerar_tabela_pessoal
# =============================================================================
# Gera uma tabela DT com os valores de despesa com pessoal, pivotada por mês,
# incluindo RPNP quando disponível.
#
# Parâmetros:
...
#   formato_numero  - Se TRUE, formata números com separador de milhar
#
# Retorno:
...
gerar_tabela_pessoal <- function(
  ...
  pessoal_prep <- data_pessoal %>% 
    ...
    mutate(
      # Formatar mês/ano em maiúsculas (ex: JAN/25)
  ...
  # Pivotar: linhas = categoria, colunas = mês
  ...
  # ---------------------------------------------------------------------------
  # 5. Adicionar coluna de TOTAL (últimos 12 meses)
  # ---------------------------------------------------------------------------
  ...
  tabela <- tabela %>%
    mutate(
      `TOTAL (ÚLTIMOS 12 MESES)` = rowSums(
...
notas <- tibble::tribble(
  ...
  "2", "Despesa Liquidada = Item de Informação 25",
  "3", "RPNP = Restos a Pagar Não Processados (Item 27)",
  "4", "Total 12 meses = Soma dos últimos 12 meses de despesa liquidada",
  "5", "Padrões de texto (APOSENT, INAT, etc.) usados para identificar despesas com inativos",
  "6", "Fontes vinculadas (023, 024, 055, 056) = Recursos previdenciários"
)

knitr::kable(notas, col.names = c("Item", "Descrição"))
```

## Observações Importantes

::: callout-warning
### Falsos Positivos na Detecção de Termos

O padrão `PEN` captura tanto **PENSÕES** (correto) quanto termos como **COMPENSAÇÕES** e **DEPENDÊNCIAS** (falsos positivos). Atenção especial deve ser dada à análise de:

-   REFORMA - pode capturar "REFORMA AGRÁRIA" (falso positivo)
-   PEN - captura "COMPENSAÇÕES" indevidamente
:::

::: callout-note
### Filtros do Tesouro Gerencial

O Tesouro Gerencial usa o operador **"Contém"** para detecção de termos, que é equivalente ao padrão sem word boundaries (`\\b`) em R. Isso pode gerar falsos positivos que devem ser tratados manualmente.
:::

# 📋 RGF Anexo 02 - Dívida Consolidada Líquida {#sec-rgf-a02}
...
ficha_tecnica <- tibble::tribble(
  ...
  "Relatório",            "RGF - Relatório de Gestão Fiscal",
  "Anexo",                "Anexo 02",
  "Nome",                 "Demonstrativo da Dívida Consolidada Líquida - DCL",
  "Base Legal",           "LRF Art. 55, inciso I, alínea 'b'",
  "Periodicidade",        "Quadrimestral",
...
# ARQUIVOS DO TESOURO GERENCIAL UTILIZADOS
# =============================================================================
# O Anexo 02 utiliza MÚLTIPLAS bases de dados com filtros específicos

arquivos <- tibble::tribble(
  ~Arquivo,                                    ~Descrição,                                    ~Filtros_TG,
  "rgf_a02_precatorios.xlsx",                  "Precatórios posteriores a 05/05/2000",        "Orç. Fiscal, Contas específicas",
  "rgf_a02_rp_exceto_170600.xlsx",             "RP processados (exceto UG 170600)",           "Contas específicas, UG != 170600, Ação != 0005",
  "rgf_a02_rp_todas_ugs.xlsx",                 "RP processados (todas UGs)",                  "Contas específicas, Ação != 0005",
  "rgf_a02_ug_170512.xlsx",                    "Dívida assumida e renegociada",               "Orç. Fiscal, UG = 170512",
  "rgf_a02_creditos_bancarios.xlsx",           "Créditos bancários",                          "Orç. Fiscal, UG IN (170705, 170526, 170700)",
  "rgf_a02_entidade.xlsx",                     "Dívida mobiliária por entidade",              "Orç. Fiscal, Contas específicas",
  "rgf_a02_aplicacao_titulos_publicos.xlsx",   "Aplicações em títulos públicos",              "Orç. Fiscal, Órgão UGE != 25901, Tipo Admin IN (3,4,5,6,8)",
  "rgf_a02_balancete.xlsx",                    "Balancete geral (principal)",                 "Orç. Fiscal",
  "rgf_a02_balancete_fat.xlsx",                "Disponibilidades FAT",                        "Orç. Fiscal, UG = 380916",
  "rgf_a02_balancete_fundos.xlsx",             "Aplicações em fundos (Tipo Admin 7)",         "Orç. Fiscal, Órgão != 25915,37904, Tipo Admin = 7",
  "rgf_a02_balancete_fundos_111215100.xlsx",   "Aplicações em fundos (conta específica)",     "Orç. Fiscal, Órgão != 25915,37904, Conta = 111215100",
  "rgf_a02_balancete_deducoes_depositos_a_vista.xlsx", "Depósitos à vista (deduções)",        "Orç. Fiscal, Órgão != 25901, UG != 380916",
  "rgf_a02_dcl_passivo_atuarial.xlsx",         "Passivo atuarial (RPPS e militares)",         "Orç. Fiscal"
)
...
atributos <- tibble::tribble(
  ~Atributo,                   ~Tipo,       ~Uso,                  ~Descrição,
  "conta_contabil_numero",     "character", "TG / Critério",       "Número da conta contábil (PCASP)",
  "conta_contabil_nome",       "character", "TG / Apresentação",   "Nome da conta contábil",
  "entidade_c_cor_numero",     "character", "TG / Critério",       "Código da entidade/conta corrente",
  "entidade_c_cor_nome",       "character", "TG / Critério",       "Nome da entidade (para detecção de padrões)",
  "conta_corrente",            "character", "TG / Critério",       "Conta corrente específica",
  "acao_governo_codigo",       "character", "TG / Critério",       "Código da ação de governo",
  "unidade_orcamentaria_codigo","character","TG / Critério",       "Código da UO",
  "ug_executora_codigo",       "character", "TG / Critério",       "Código da UG executora",
  "isf_lancamento",            "character", "TG / Critério",       "Indicador de Superávit Financeiro (P=Primário)",
  "c_con_subgrupo_3_nome",     "character", "TG / Critério",       "Nome do subgrupo contábil",
  "mes_lancamento",            "numeric",   "TG / R",              "Mês do lançamento (YYYYMM)",
  "saldo_r_conta_contabil",    "numeric",   "TG / R",              "Saldo da conta contábil",
  "data_lancamento",           "Date",      "R",                   "Data calculada do lançamento"
)
...
vinculos <- tibble::tribble(
  ~Anexo_Relacionado,   ~Tipo_Vínculo,    ~Descrição,
  "RREO Anexo 03",      "Referência",     "RCL utilizada para cálculo do limite da DCL",
  "RGF Anexo 05",       "Referência",     "Disponibilidades para cálculo de liquidez",
  "RGF Anexo 06",       "Consolidação",   "Valores consolidados no demonstrativo simplificado"
)
...
estrutura <- tibble::tribble(
  ~Ordem, ~Item,                                           ~Sinal,
  "01",   "Dívida Mobiliária TN Interna",                   "+",
  "02",   "Aplicação em Títulos Públicos (dedução)",        "-",
  "03",   "Dívida Mobiliária TN em BCB",                    "+",
  "04",   "Dívida Securitizada",                            "+",
  "05",   "Dívida Mobiliária Externa",                      "+",
  "06",   "Operações de Equalização Cambial",               "+",
  "07",   "Demais Dívidas Contratuais",                     "+",
  "08",   "Precatórios posteriores a 05/05/2000",           "+",
  "09",   "Dívida Assumida pela União",                     "+",
  "10",   "Passivos por Insuficiência de Recursos",         "+",
  "11",   "Depósitos do TN em BCB (dedução)",               "-",
  "12",   "Depósitos à Vista (dedução)",                    "-",
  "13",   "Disponibilidades FAT (dedução)",                 "-",
  "14",   "Aplicações em Fundos Diversos 1 (dedução)",      "-",
  "15",   "Aplicações em Fundos Diversos 2 (dedução)",      "-",
  "16",   "Aplicações em Fundos Diversos 3 (dedução)",      "-",
  "17",   "Dívidas Renegociadas (dedução)",                 "-",
  "18",   "Créditos Lei 8.727/93 (dedução)",                "-",
  "19",   "Dívida Externa Renegociada (dedução)",           "-",
  "20",   "Demais Dívidas Renegociadas (dedução)",          "-",
  "21",   "Ajustes para Perdas (positivo)",                 "+",
  "22",   "Outros Créditos Bancários (dedução)",            "-",
  "23",   "Outros Créditos Bancários - Ajustes",            "+",
  "24",   "Resultado Positivo TN/BCB (dedução)",            "-",
  "25",   "Restos a Pagar Processados",                     "+",
  "26",   "Dívida Mobiliária TN Interna INTRA (dedução)",   "-",
  "38",   "Precatórios (ajuste s/ ações 0005 e 0EC7)",      "-",
  "40",   "RPPS Civil (passivo atuarial)",                  "+",
  "41",   "Despesas Previdenciárias FCDF",                  "+",
  "42",   "Militares Inativos",                             "+",
  "43",   "Pensões Militares",                              "+"
)
...
#| code-fold: true
#| code-summary: "Ver critérios - Precatórios"
...
criterios_rgf_A02_dcl_precatorios <- list(
  ...
  # 08 - PRECATÓRIOS POSTERIORES A 05/05/2000
  # ---------------------------------------------------------------------------
  # Inclui ações de precatórios, exceto conta 622130400
  # Ação 0Z01 apenas se UO = 71103
  ...
  # ---------------------------------------------------------------------------
  # 38 - PRECATÓRIOS (AJUSTE SEM AÇÕES 0005 e 0EC7)
  # ---------------------------------------------------------------------------
  # Conta específica 622130400 com ações 0005 e 0EC7
  
  `38 Precatórios posteriores a 05/05/2000 sem ação 0005 e 0EC7` = list(
    criterio = "
    conta_contabil_numero == c('622130400') & 
...
```{r criterios-rp}
#| code-fold: true
#| code-summary: "Ver critérios - Restos a Pagar"
...
#| code-fold: true
#| code-summary: "Ver critérios - UG 170512"

# =============================================================================
# BASE: rgf_a02_ug_170512
# Filtros TG: Orçamento Fiscal, UG = 170512 (Secretaria do Tesouro Nacional)
# =============================================================================
...
### Base: Créditos Bancários

```{r criterios-creditos-bancarios}
#| code-fold: true
#| code-summary: "Ver critérios - Créditos Bancários"
...
criterios_rgf_A02_dcl_creditos_bancarios <- list(
  ...
  # ---------------------------------------------------------------------------
  # 23 - OUTROS CRÉDITOS BANCÁRIOS - AJUSTES (POSITIVO)
  # ---------------------------------------------------------------------------
...
### Base: Entidade (Dívida Mobiliária)

```{r criterios-entidade}
#| code-fold: true
#| code-summary: "Ver critérios - Entidade"
...
criterios_rgf_A02_dcl_entidade <- list(
  ...
  # 01 - DÍVIDA MOBILIÁRIA TN INTERNA
  # ---------------------------------------------------------------------------
  # Títulos públicos federais (LTN, LFT, NTN-B, NTN-F, etc.)
  ...
  # 03 - DÍVIDA MOBILIÁRIA TN EM BCB
  # ---------------------------------------------------------------------------
  # Títulos em carteira do Banco Central
  ...
  # 04 - DÍVIDA SECURITIZADA
  # ---------------------------------------------------------------------------
  # Títulos de securitização e TDA
...
### Base: Aplicação em Títulos Públicos

```{r criterios-aplicacao-titulos}
#| code-fold: true
#| code-summary: "Ver critérios - Aplicação em Títulos"
...
criterios_rgf_A02_dcl_aplicacao_titulos_publicos <- list(
  ...
  # 02 - APLICAÇÃO EM TÍTULOS PÚBLICOS (DEDUÇÃO)
  # ---------------------------------------------------------------------------
  # Exclui contas específicas que não devem ser deduzidas
...
#| code-fold: true
#| code-summary: "Ver critérios - Balancete"

# =============================================================================
# BASE: rgf_a02_balancete (renomeado de rgf_a02_balancete)
# Filtros TG: Orçamento Fiscal
# NOTA: Esta base é usada em diversos anexos além do RGF A02
# =============================================================================
...
criterios_rgf_A02_dcl_balancete <- list(
  ...
  # ---------------------------------------------------------------------------
  # 12 - DEPÓSITOS À VISTA (DEDUÇÃO) - PARTE 1
  # ---------------------------------------------------------------------------
...
#| code-fold: true
#| code-summary: "Ver critérios - Balancete FAT"
...
#| code-fold: true
#| code-summary: "Ver critérios - Balancete Fundos"
...
#| code-fold: true
#| code-summary: "Ver critérios - Passivo Atuarial"
...
criterios_rgf_A02_dcl_passivo_atuarial <- list(
  ...
  # ---------------------------------------------------------------------------
  # 41 - DESPESAS PREVIDENCIÁRIAS DO FCDF
  # ---------------------------------------------------------------------------
...
# =============================================================================
# Cada base de dados tem seus critérios aplicados separadamente
# =============================================================================

cat("📊 Processando critérios do RGF Anexo 02...\n")
...
cat("✅ Restos a Pagar processados\n")
...
cat("✅ UG 170512 processada\n")

# -----------------------------------------------------------------------------
# 4. Créditos Bancários
# -----------------------------------------------------------------------------
...
cat("✅ Créditos bancários processados\n")

# -----------------------------------------------------------------------------
# 5. Entidade (Dívida Mobiliária)
# -----------------------------------------------------------------------------
...
cat("✅ Balancete processado\n")

# -----------------------------------------------------------------------------
# 7. Aplicação em Títulos Públicos
# -----------------------------------------------------------------------------
...
cat("✅ Aplicação em títulos processada\n")
...
cat("✅ Balancete Fundos processado\n")

# -----------------------------------------------------------------------------
# 10. Depósitos à Vista (deduções)
# -----------------------------------------------------------------------------
...
cat("✅ Depósitos à vista processados\n")
...
cat("✅ Passivo atuarial processado\n")
```
...
#| code-fold: true
#| code-summary: "Ver código de consolidação"
...
cat(sprintf("✅ Consolidação concluída: %d linhas\n", nrow(df_rgf_A02_dcl_consolidado)))
```
...
#| code-fold: true
#| code-summary: "Ver código de ajuste de sinais"
...
```{r calculos-dcl}
#| code-fold: true
#| code-summary: "Ver cálculos da DCL"

# =============================================================================
# CÁLCULOS AGREGADOS DA DCL
# =============================================================================

# Dívida Mobiliária = 01 + 02 + 03 + 04 + 05 + 26
divida_mobiliaria <- valores_linhas %>% 
  filter(ordem %in% c("01", "02", "03", "04", "05", "26")) %>% 
  summarise(valor = sum(valor, na.rm = TRUE)) %>%
  pull(valor)

# Dívida Consolidada = Dívida Mobiliária + 06 + 07 + 08 + 09 + 10 + 38
...
cat("\n📊 RESUMO DA DCL\n")
cat("================\n")
cat(sprintf("Dívida Mobiliária:   R$ %s\n", formatar_numero(divida_mobiliaria)))
cat(sprintf("Dívida Consolidada:  R$ %s\n", formatar_numero(divida_consolidada)))
cat(sprintf("Precatórios:         R$ %s\n", formatar_numero(precatorios)))
cat(sprintf("Deduções TN/BCB:     R$ %s\n", formatar_numero(deducoes_tn_bcb)))
...
notas <- tibble::tribble(
  ...
  "2", "ISF = P indica item Primário (relevante para DCL)",
  "3", "Linhas de dedução (-) são subtraídas da dívida bruta",
  "4", "RP Processados consolida dados de duas bases (exceto 170600 + todas UGs)",
  "5", "Depósitos à vista consolida dados do balancete geral + base específica",
  "6", "Passivo atuarial inclui RPPS civil, FCDF e militares",
  "7", "Dívida mobiliária inclui todos os títulos públicos federais"
)
...
O RGF Anexo 02 é o demonstrativo mais complexo, utilizando **13 bases de dados diferentes** do Tesouro Gerencial, cada uma com filtros específicos. A correta extração dos dados no TG é fundamental para a precisão dos cálculos.
:::

::: callout-note
### Sobre o Balancete

O arquivo `rgf_a02_balancete.xlsx` (renomeado para "balancete") é utilizado em diversos anexos além do RGF A02. Por isso, foi sugerida a renomeação para um nome mais genérico que reflita seu uso compartilhado.
:::

::: callout-tip
### Verificação dos Resultados

Para validar os valores, compare com o demonstrativo oficial publicado no SICONFI. As principais fontes de divergência são:

-   Diferenças de data/mês de referência
-   Filtros incorretos na extração do TG
-   Contas contábeis atualizadas no PCASP
:::

# 📋 RGF Anexo 03 - Garantias e Contragarantias {#sec-rgf-a03}
...
ficha_tecnica <- tibble::tribble(
  ...
  "Base Legal",           "LRF Art. 55, inciso I, alínea 'c' e Art. 40, § 1º",
  "Periodicidade",        "Quadrimestral",
...
arquivos <- tibble::tribble(
  ~Arquivo,              ~Descrição,                                    ~Filtros_TG,
  "rgf_a03.xlsx",        "Garantias e contragarantias de valores",      "Orçamento Fiscal, Contas 81* e 89*"
)
...
atributos <- tibble::tribble(
  ...
  "conta_contabil_numero",     "character", "TG / Critério",       "Número da conta contábil (PCASP)",
  "conta_contabil_nome",       "character", "TG / Apresentação",   "Nome da conta contábil",
  "conta_corrente",            "character", "TG / Critério",       "Conta corrente (identifica beneficiário)",
  "mes_lancamento",            "numeric",   "TG / R",              "Mês do lançamento (YYYYMM)",
  "saldo_r_conta_contabil",    "numeric",   "TG / R",              "Saldo da conta contábil",
  "data_lancamento",           "Date",      "R",                   "Data calculada do lançamento"
)
...
vinculos <- tibble::tribble(
  ~Anexo_Relacionado,   ~Tipo_Vínculo,    ~Descrição,
  "RREO Anexo 03",      "Referência",     "RCL utilizada para cálculo do limite de garantias",
  "RGF Anexo 06",       "Consolidação",   "Valores consolidados no demonstrativo simplificado"
)
...
estrutura <- tibble::tribble(
  ...
  "04",   "Garantias aos Municípios - Total",              "I - Garantias Concedidas",
  "05",   "Garantias aos Municípios - Op. Externas",       "I - Garantias Concedidas",
  "06",   "Garantias aos Municípios - Op. Internas",       "I - Garantias Concedidas",
  "07",   "Garantias às Entidades Controladas - Total",    "I - Garantias Concedidas",
  "08",   "Garantias às Entidades - Op. Externas",         "I - Garantias Concedidas",
  "09",   "Garantias às Entidades - Op. Internas",         "I - Garantias Concedidas",
  ...
  "18",   "Contragarantias dos Municípios - Total",        "II - Contragarantias Recebidas",
  "19",   "Contragarantias dos Municípios - Op. Externas", "II - Contragarantias Recebidas",
  "20",   "Contragarantias dos Municípios - Op. Internas", "II - Contragarantias Recebidas",
...
## Contas Correntes de Referência

```{r contas-correntes-rgf-a03}
#| echo: false

# =============================================================================
# TABELA DE CONTAS CORRENTES (BENEFICIÁRIOS)
# =============================================================================

contas_correntes <- tibble::tribble(
  ~Conta_Corrente,  ~Beneficiário,                    ~Tipo_Operação,
  "CG0000064",      "Estados",                        "Operações Internas",
  "CG0000069",      "Estados",                        "Operações Externas",
  "CG0000065",      "Municípios",                     "Operações Internas",
  "CG0000070",      "Municípios",                     "Operações Externas",
  "CG0000067",      "Entidades Controladas",          "Operações Internas",
  "CG0000071",      "Entidades Controladas",          "Operações Externas (tipo 1)",
  "CG0000072",      "Entidades Controladas",          "Operações Externas (tipo 2)",
  "CGASCA001",      "Fundos - ASCA",                  "Programas",
  "CGLEI8036",      "Fundos - Lei 8.036 (FGTS)",      "Programas"
)

knitr::kable(contas_correntes, col.names = c("Conta Corrente", "Beneficiário", "Tipo de Operação"))
```
...
# =============================================================================
# CRITÉRIOS RGF ANEXO 03 - GARANTIAS E CONTRAGARANTIAS
# =============================================================================
...
criterios_rgf_A03_garantias <- list(
  ...
  # ---------------------------------------------------------------------------
  # GARANTIAS ÀS ENTIDADES CONTROLADAS
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # CONTRAGARANTIAS DOS MUNICÍPIOS
  # ---------------------------------------------------------------------------
...
cat("✅ Critérios RGF Anexo 03 definidos\n")
...
cat("📊 Processando critérios do RGF Anexo 03...\n")
...
cat(sprintf("✅ Critérios aplicados: %d registros\n", nrow(garantias_processado)))
...
# =============================================================================
# RESUMO DAS GARANTIAS POR BENEFICIÁRIO
# =============================================================================

# Garantias por beneficiário
...
# Contragarantias por beneficiário
...
notas <- tibble::tribble(
  ...
  "5", "Conta corrente identifica o beneficiário da garantia",
  "6", "Operações externas = financiamentos internacionais",
  "7", "Operações internas = financiamentos nacionais"
)
...
O limite de garantias conforme a LRF é de **60% da RCL** para a União. O valor atual deve ser comparado com este limite no RGF Anexo 06 (Demonstrativo Simplificado).
:::

::: callout-tip
### Contas Contábeis

As principais contas contábeis utilizadas são:
...
# 📋 RGF Anexo 04 - Operações de Crédito {#sec-rgf-a04}
...
ficha_tecnica <- tibble::tribble(
  ...
  "Relatório",            "RGF - Relatório de Gestão Fiscal",
  "Anexo",                "Anexo 04",
  "Nome",                 "Demonstrativo das Operações de Crédito",
  "Base Legal",           "LRF Art. 55, inciso I, alínea 'd' e Art. 32",
  "Periodicidade",        "Quadrimestral",
...
arquivos <- tibble::tribble(
  ~Arquivo,                    ~Descrição,                                    ~Filtros_TG,
  "rgf_a04_receitas.xlsx",     "Receitas de operações de crédito",            "Orç. Fiscal, Contas 621*",
  "rgf_a04_despesas.xlsx",     "Aplicação dos recursos (despesas)",           "Orç. Fiscal, Contas 622*",
  "rgf_a02_balancete.xlsx",    "Operações extraorçamentárias",                "Orç. Fiscal, Contas 896* e 212*"
)
...
atributos <- tibble::tribble(
  ...
  "conta_contabil_numero",             "character", "TG / Critério",       "Número da conta contábil (PCASP)",
  "natureza_receita_codigo_completo",  "character", "TG / Critério",       "Código completo da natureza de receita",
  "grupo_despesa_codigo_grupo",        "character", "TG / Critério",       "Código do grupo de despesa (1-6)",
  "mes_lancamento",                    "numeric",   "TG / R",              "Mês do lançamento (YYYYMM)",
  "saldo_r_conta_contabil",            "numeric",   "TG / R",              "Saldo da conta contábil",
  "movim_liquido_r_item_informacao",   "numeric",   "TG / R",              "Movimento líquido",
  "data_lancamento",                   "Date",      "R",                   "Data calculada do lançamento"
)
...
vinculos <- tibble::tribble(
  ~Anexo_Relacionado,   ~Tipo_Vínculo,    ~Descrição,
  "RREO Anexo 03",      "Referência",     "RCL utilizada para cálculo do limite de operações",
  "RGF Anexo 02",       "Referência",     "Balancete compartilhado para extraorçamentárias",
  "RGF Anexo 06",       "Consolidação",   "Valores consolidados no demonstrativo simplificado"
)
...
# =============================================================================
# ESTRUTURA DO DEMONSTRATIVO - OPERAÇÕES DE CRÉDITO
# =============================================================================

estrutura <- tibble::tribble(
  ~Ordem, ~Item,                                                    ~Grupo,
  "01",   "Mobiliária Interna - Refinanciamento",                   "I - Sujeitas ao Limite",
  "02",   "Mobiliária Interna - Outras Orçamentárias",              "I - Sujeitas ao Limite",
  "03",   "Mobiliária Interna - Extraorç. Aporte Bacen",            "I - Sujeitas ao Limite",
  "04",   "Mobiliária Interna - Extraorç. Aporte Empresas",         "I - Sujeitas ao Limite",
  "05",   "Mobiliária Interna - Extraorç. Trocas e Demais",         "I - Sujeitas ao Limite",
  "06",   "Mobiliária Externa - Refinanciamento",                   "I - Sujeitas ao Limite",
  "07",   "Mobiliária Externa - Assunção/Reconhecimento Dívidas",   "I - Sujeitas ao Limite",
  "08",   "Mobiliária Externa - Outras Op. Mobiliárias",            "I - Sujeitas ao Limite",
  "09",   "Contratual Interna - Abertura de Crédito",               "I - Sujeitas ao Limite",
  "10",   "Contratual Interna - Assunção/Reconhecimento Dívidas",   "I - Sujeitas ao Limite",
  "11",   "Contratual Interna - Outras Op. Contratuais",            "I - Sujeitas ao Limite",
  "12",   "Contratual Externa - Abertura Crédito Orçamentárias",    "I - Sujeitas ao Limite",
  "13",   "Contratual Externa - Abertura Crédito Extraorç.",        "I - Sujeitas ao Limite",
  "14",   "Contratual Externa - Assunção/Reconhecimento Dívidas",   "I - Sujeitas ao Limite",
  "15",   "Contratual Externa - Outras Op. Contratuais",            "I - Sujeitas ao Limite",
  "16",   "Amortização/Refinanciamento",                            "II - Aplicação dos Recursos",
  "17",   "Outras Despesas Correntes",                              "II - Aplicação dos Recursos",
  "18",   "Pessoal e Encargos Sociais",                             "II - Aplicação dos Recursos",
  "19",   "Juros e Encargos da Dívida",                             "II - Aplicação dos Recursos",
  "20",   "Investimentos",                                          "II - Aplicação dos Recursos",
  "21",   "Inversões Financeiras",                                  "II - Aplicação dos Recursos"
)
...
#| code-fold: true
#| code-summary: "Ver critérios - Receitas Orçamentárias"

# =============================================================================
# PARTE 1: RECEITAS ORÇAMENTÁRIAS
# Base: rgf_a04_receitas
# Contas contábeis: 621* (Receitas de Operações de Crédito)
# =============================================================================
...
cat("✅ Critérios de Receitas definidos\n")
```

### Parte 2: Operações Extraorçamentárias

```{r criterios-extra-rgf-a04}
#| code-fold: true
#| code-summary: "Ver critérios - Extraorçamentárias"

# =============================================================================
# PARTE 2: OPERAÇÕES EXTRAORÇAMENTÁRIAS
# Base: rgf_a02_balancete (compartilhado com RGF A02)
# Contas contábeis: 896* (Emissões extraorçamentárias) e 212*
# =============================================================================

criterios_rgf_A04_extra_orcamentarias <- list(
  
  # ---------------------------------------------------------------------------
  # MOBILIÁRIA INTERNA - EXTRAORÇAMENTÁRIAS
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # MOBILIÁRIA EXTERNA - EXTRAORÇAMENTÁRIAS
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # CONTRATUAL INTERNA - EXTRAORÇAMENTÁRIAS
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # CONTRATUAL EXTERNA - EXTRAORÇAMENTÁRIAS
  # ---------------------------------------------------------------------------
...
cat("✅ Critérios Extraorçamentários definidos\n")
```

### Parte 3: Aplicação dos Recursos (Despesas)

```{r criterios-despesas-rgf-a04}
#| code-fold: true
#| code-summary: "Ver critérios - Despesas"

# =============================================================================
# PARTE 3: APLICAÇÃO DOS RECURSOS (DESPESAS)
# Base: rgf_a04_despesas
# Contas contábeis: 622* (Execução de Despesa)
# =============================================================================

criterios_rgf_A04_despesas <- list(
  
  # ---------------------------------------------------------------------------
  # APLICAÇÃO DOS RECURSOS POR GRUPO DE DESPESA
  # ---------------------------------------------------------------------------
...
cat("✅ Critérios de Despesas definidos\n")
```
...
cat("📊 Processando critérios do RGF Anexo 04...\n")
...
cat(sprintf("✅ Receitas processadas: %d registros\n", nrow(df_criterios_rgf_A04_receitas)))

# -----------------------------------------------------------------------------
# 2. Operações Extraorçamentárias
# -----------------------------------------------------------------------------
...
cat(sprintf("✅ Extraorçamentárias processadas: %d registros\n", 
            nrow(df_criterios_rgf_A04_extra_orcamentarias)))

# -----------------------------------------------------------------------------
# 3. Despesas (Aplicação dos Recursos)
# -----------------------------------------------------------------------------
...
cat(sprintf("✅ Despesas processadas: %d registros\n", nrow(df_criterios_rgf_A04_despesas)))
```
...
# Total de Operações de Crédito (linhas 01-15)
valor_operacoes_credito_total <- operacoes_credito_agregado %>%
  filter(as.numeric(ordem) <= 15) %>%
  summarise(valor = sum(valor, na.rm = TRUE)) %>%
  pull(valor)

# Total de Aplicação dos Recursos (linhas 16-21)
...
### Aplicação dos Recursos (Despesas)
...
```{r resumo-rgf-a04}
#| code-fold: true
#| code-summary: "Ver resumo das operações"
...
cat("OPERAÇÕES DE CRÉDITO:\n")
cat(sprintf("  Mobiliária Interna:   R$ %s\n", formatar_numero(mobiliaria_interna)))
cat(sprintf("  Mobiliária Externa:   R$ %s\n", formatar_numero(mobiliaria_externa)))
...
notas <- tibble::tribble(
  ~Item, ~Descrição,
  "1", "Valores em R$ (reais)",
  "2", "Contas 621* = Receitas de operações de crédito (orçamentárias)",
  "3", "Contas 622* = Execução de despesa (aplicação dos recursos)",
  "4", "Contas 896* = Emissões extraorçamentárias de títulos",
  "5", "Operações mobiliárias = títulos públicos",
  "6", "Operações contratuais = empréstimos e financiamentos",
  "7", "O balancete (rgf_a02_balancete) é compartilhado com o RGF Anexo 02"
)
...
::: callout-note
### Limite de Operações de Crédito (LRF)

O limite de operações de crédito conforme a LRF é de **16% da RCL** para a União. O valor deve ser comparado com este limite no RGF Anexo 06 (Demonstrativo Simplificado).
:::

::: callout-warning
### Operações Extraorçamentárias

As operações extraorçamentárias (linhas 03-05, 07, 10, 13-14) utilizam dados do **balancete** (rgf_a02_balancete), que é compartilhado com o RGF Anexo 02. Certifique-se de que este arquivo foi importado antes de processar o Anexo 04.
:::

::: callout-tip
### Naturezas de Receita

As principais naturezas de receita utilizadas são:

-   **2111**: Operações de Crédito Internas - Mobiliárias
-   **2112**: Operações de Crédito Internas - Contratuais
-   **2121**: Operações de Crédito Externas - Mobiliárias
-   **2122**: Operações de Crédito Externas - Contratuais
:::

# 📋 RGF Anexo 05 - Disponibilidades de Caixa {#sec-rgf-a05}
...
ficha_tecnica <- tibble::tribble(
  ...
  "Base Legal",           "LRF Art. 55, inciso III, alínea 'a'",
  "Periodicidade",        "Quadrimestral",
  "Grau de Dificuldade",  "⭐⭐⭐⭐⭐ Muito Difícil",
  "Última Atualização",   format(Sys.Date(), "%d/%m/%Y")
)
...
arquivos <- tibble::tribble(
  ~Arquivo,                              ~Descrição,                                    ~Filtros_TG,
  "rgf_a05_111_ate_fonte_002.xlsx",      "Disponibilidades fontes 000-002",             "Orç. Fiscal, Fontes 000-002",
  "rgf_a05_111_fonte_003_052.xlsx",      "Disponibilidades fontes 003-052",             "Orç. Fiscal, Fontes 003-052",
  "rgf_a05_111_fonte_053_em_diante_executivo.xlsx", "Disponibilidades fontes 053+ (Exec.)", "Orç. Fiscal, Fontes 053+, Poder Executivo",
  "rgf_a05_111_fonte_053_em_diante_demais.xlsx",    "Disponibilidades fontes 053+ (Demais)", "Orç. Fiscal, Fontes 053+, Demais Poderes",
  "rgf_a05_21.xlsx",                     "Passivos circulantes",                        "Orç. Fiscal, Contas 21*",
  "rgf_a05_22_23.xlsx",                  "Passivos não circulantes",                    "Orç. Fiscal, Contas 22* e 23*",
  "rgf_a05_contas_contabeis.xlsx",       "Contas contábeis específicas",                "Orç. Fiscal, Contas específicas",
  "disponibilidade_inicial.xlsx",         "Disponibilidade inicial (arquivo auxiliar)",  "Planilha Excel com abas União e Executivo"
)
...
atributos <- tibble::tribble(
  ...
  "conta_contabil_numero",                      "character", "TG / Critério",       "Número da conta contábil (PCASP)",
  "fonte_recursos_codigo",                      "character", "TG / Critério",       "Código da fonte de recursos",
  "isf_lancamento",                             "character", "TG / Critério",       "Indicador de Superávit Financeiro (F/P)",
  "ug_executora_codigo",                        "character", "TG / Critério",       "Código da UG executora",
  "orgao_c_cor_codigo",                         "character", "TG / Critério",       "Código do órgão (conta corrente)",
  "orgao_uge_poder_codigo",                     "character", "TG / Critério",       "Código do poder (0=Executivo)",
  "orgao_uge_orgao_maximo_codigo",              "character", "TG / Critério",       "Código do órgão máximo",
  "detalhe_orgao_central_codigo_detalhe_oc",    "character", "TG / Critério",       "Código detalhe órgão central",
  "detalhe_orgao_central_id_fonte",             "character", "TG / Critério",       "ID fonte órgão central",
  "mes_lancamento",                             "numeric",   "TG / R",              "Mês do lançamento (YYYYMM)",
  "saldo_r_conta_contabil",                     "numeric",   "TG / R",              "Saldo da conta contábil",
  "data_lancamento",                            "Date",      "R",                   "Data calculada do lançamento"
)
...
vinculos <- tibble::tribble(
  ~Anexo_Relacionado,   ~Tipo_Vínculo,    ~Descrição,
  "RGF Anexo 02",       "Referência",     "Balancete para algumas contas específicas",
  "RGF Anexo 06",       "Consolidação",   "Valores consolidados no demonstrativo simplificado"
)
...
estrutura <- tibble::tribble(
  ~Código,                        ~Linha,                              ~Fontes,
  "0_nao_vinculados",             "Recursos Não Vinculados",           "000",
  "1_educacao",                   "Educação",                          "008, 012, 130, 133, 134, 142, 143, 151",
  "2_seguridade_exceto_previdencia", "Seguridade (exceto Previdência)", "001, 002, 004, 005, 006, 010, 017, 023, 024, 035, 040, ...",
  "3_rpps",                       "RPPS",                              "055, 056, 125",
  "4_rgps",                       "RGPS",                              "054",
  "5_divida",                     "Dívida",                            "400, 401, 443, 444, 448, + específicas",
  "6_transferencias",             "Transferências",                    "201-289",
  "7_fundos_orgaos_programa",     "Fundos, Órgãos e Programas",        "003-199 (exceto anteriores)",
  "8_extraorcamentario",          "Extraorçamentário",                 "491",
  "9_nao_classificados",          "Recursos a Classificar",            "490, -9, -7"
)

knitr::kable(estrutura, col.names = c("Código", "Linha", "Fontes de Recursos"))
```
...
colunas <- tibble::tribble(
  ~Coluna,                                    ~Descrição,
  "Disponibilidade Inicial",                  "Saldo inicial do exercício (arquivo auxiliar)",
  "Receitas",                                 "Receitas orçamentárias realizadas",
  "Despesas",                                 "Despesas orçamentárias pagas",
  "Demais Fluxos",                            "Ajustes e outros fluxos",
  "Disponibilidade Bruta",                    "Caixa bruto antes de obrigações",
  "RP Liquidados NP - Exercícios Anteriores", "Restos a pagar liquidados não pagos (anos anteriores)",
  "RP Liquidados NP - Do Exercício",          "Restos a pagar liquidados não pagos (ano atual)",
  "RP Empenhados NL - Exercícios Anteriores", "Restos a pagar empenhados não liquidados",
  "Demais Obrigações Financeiras",            "Outras obrigações financeiras",
  "Disponibilidade Antes Inscrição RP",       "Disponibilidade líquida final"
)

knitr::kable(colunas, col.names = c("Coluna", "Descrição"))
```
...
```{r preparar-rgf-a05}
#| code-fold: true
#| code-summary: "Ver código de preparação"
...
# -----------------------------------------------------------------------------
# Criar chaves compostas para identificação
# -----------------------------------------------------------------------------

rgf_a05 <- rgf_a05 %>% 
  mutate(
    # Chave órgão + fonte completa
    orgao_fonte = str_c(
    ...
    # Chave órgão + fonte (sem código fonte)
  ...
  rgf_a05 <- rgf_a05 %>%
    mutate(
      linhas = case_when(
        ...
        # Fundos, Órgãos e Programas (demais fontes)
        fonte_recursos_codigo %in% c(
        ...
        # Não Classificados / A Classificar
...
cat("✅ Dados preparados com classificação de linhas\n")
...
# =============================================================================
# CRITÉRIOS RGF ANEXO 05 - DISPONIBILIDADE DE CAIXA E RESTOS A PAGAR
# =============================================================================
# 22 critérios para classificação das contas contábeis
# =============================================================================
...
cat("✅ Critérios RGF Anexo 05 definidos\n")
...
cat("📊 Processando critérios do RGF Anexo 05...\n")

# -----------------------------------------------------------------------------
# Função auxiliar para obter coluna (com fallback para 0)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Criar abrangências: União e Executivo
# -----------------------------------------------------------------------------
...
```{r calcular-rgf-a05}
#| code-fold: true
#| code-summary: "Ver código de cálculos"

# =============================================================================
# CÁLCULOS DO DEMONSTRATIVO
# =============================================================================
...
# -----------------------------------------------------------------------------
# Calcular Demais Obrigações Financeiras Clean
# -----------------------------------------------------------------------------
...
if (file.exists(arquivo_disp_inicial)) {
  ...
  cat("✅ Disponibilidade inicial carregada\n")
} else {
  # Criar coluna com zeros se arquivo não existir
  rgf_a05_clean <- rgf_a05_clean %>%
    mutate(disponibilidade_inicial = 0)
  
  cat("⚠️ Arquivo disponibilidade_inicial.xlsx não encontrado - usando zeros\n")
}
...
#| code-fold: true
#| code-summary: "Ver código de resultados"
...
cat(sprintf("\n✅ Disponibilidade Final - União: R$ %s\n", 
            formatar_numero(valor_disponibilidade_uniao)))
cat(sprintf("✅ Disponibilidade Final - Executivo: R$ %s\n", 
            formatar_numero(valor_disponibilidade_executivo)))
```

### Tabela União (Todos os Poderes)
...
notas <- tibble::tribble(
  ...
  "3", "União inclui todos os poderes e órgãos",
  "4", "Executivo exclui DPU (34000) e MPU (59000)",
  "5", "Disponibilidade inicial vem de arquivo auxiliar externo",
  "6", "Linhas são classificadas por fonte de recursos",
  "7", "TEDs são ajustados na linha 'Fundos, Órgãos e Programas'",
  "8", "Recursos a Classificar têm dedução específica"
)
...
O RGF Anexo 05 é um dos anexos mais complexos, envolvendo:

-   **Múltiplos arquivos** segmentados por fonte de recursos
-   **Classificação de linhas** baseada em fontes
-   **Duas abrangências** (União e Executivo)
...
-   **uniao**: Disponibilidade inicial para a União
...
-   **631**\*: Restos a Pagar não processados
-   **632**\*: Restos a Pagar processados
-   **894**\*: TEDs a liberar/receber
:::

# 📋 RREO Anexo 01 - Balanço Orçamentário {#sec-rreo-a01}
...
ficha_tecnica <- tibble::tribble(
  ~Campo,                 ~Informação,
  "Relatório",            "RREO - Relatório Resumido da Execução Orçamentária",
  "Anexo",                "Anexo 01",
  "Nome",                 "Balanço Orçamentário",
  "Base Legal",           "LRF Art. 52, inciso I e II, alíneas 'a' e 'b'",
...
arquivos <- tibble::tribble(
  ~Arquivo,                              ~Descrição,                          ~Filtros_TG,
  "tg_receita_liquida.xlsx",             "Receitas realizadas",               "Orç. Fiscal, Item Informação",
  "tg_receita_previsao.xlsx",            "Previsão de receitas",              "Orç. Fiscal, Item Informação",
  "tg_despesa_dotacao_inicial.xlsx",     "Dotação inicial de despesas",       "Orç. Fiscal",
  "tg_despesa_dotacao_atualizada_*.xlsx", "Dotação atualizada (partes)",      "Orç. Fiscal",
  "tg_despesa_despesas_empenhadas_*.xlsx", "Despesas empenhadas (partes)",    "Orç. Fiscal",
  "tg_despesa_despesas_liquidadas.xlsx", "Despesas liquidadas",               "Orç. Fiscal",
  "tg_despesa_pagamentos_totais.xlsx",   "Despesas pagas",                    "Orç. Fiscal"
)
...
atributos_receitas <- tibble::tribble(
  ~Atributo,                               ~Tipo,       ~Uso,                  ~Descrição,
  "nre1_categoria_economica_codigo",       "numeric",   "TG / Critério",       "Categoria econômica (1=Corrente, 2=Capital, 7/8=Intra)",
  "nre2_origem_receita_codigo_origem",     "numeric",   "TG / Critério",       "Origem da receita (1-9)",
  "nre3_especie_receita_codigo_especie",   "numeric",   "TG / Critério",       "Espécie da receita",
  "natureza_receita_codigo_completo",      "character", "TG / Critério",       "Código completo da natureza de receita",
  "item_informacao_codigo",                "character", "TG / Critério",       "Código do item (1=Prev.Inicial, 2=Prev.Atualizada, 5=Realizada)",
  "item_informacao_nome",                  "character", "TG / Apresentação",   "Nome do item de informação",
  "mes_lancamento",                        "numeric",   "TG / R",              "Mês do lançamento (YYYYMM)",
  "saldo_r_item_informacao",               "numeric",   "TG / R",              "Saldo do item de informação"
)

knitr::kable(atributos_receitas, col.names = c("Atributo", "Tipo", "Uso", "Descrição"))
```
...
atributos_despesas <- tibble::tribble(
  ~Atributo,                               ~Tipo,       ~Uso,                  ~Descrição,
  "categoria_economica_despesa_codigo",    "numeric",   "TG / Critério",       "Categoria econômica (3=Corrente, 4=Capital, 9=Reserva)",
  "grupo_despesa_codigo_grupo",            "numeric",   "TG / Critério",       "Grupo de despesa (1-6)",
  "modalidade_aplicacao_codigo",           "character", "TG / Critério",       "Modalidade de aplicação (91=Intra)",
  "elemento_despesa_codigo",               "character", "TG / Critério",       "Elemento de despesa",
  "subfuncao_governo_codigo",              "character", "TG / Critério",       "Subfunção de governo",
  "fonte_recursos_codigo",                 "character", "TG / Critério",       "Fonte de recursos",
  "unidade_orcamentaria_codigo",           "character", "TG / Critério",       "Código da unidade orçamentária",
  "item_informacao_codigo",                "character", "TG / Critério",       "Código do item de informação",
  "mes_lancamento",                        "numeric",   "TG / R",              "Mês do lançamento (YYYYMM)",
  "saldo_r_item_informacao",               "numeric",   "TG / R",              "Saldo do item de informação"
)

knitr::kable(atributos_despesas, col.names = c("Atributo", "Tipo", "Uso", "Descrição"))
```
...
vinculos <- tibble::tribble(
  ~Anexo_Relacionado,   ~Tipo_Vínculo,    ~Descrição,
  "RREO Anexo 03",      "Referência",     "RCL utiliza receitas correntes",
  "RREO Anexo 06",      "Referência",     "Resultado primário utiliza receitas e despesas",
  "RREO Anexo 07",      "Referência",     "Restos a pagar utiliza despesas"
)
...
estrutura_receitas <- tibble::tribble(
  ...
  "02",   "Impostos, Taxas e Contribuições de Melhoria",   "Receitas Correntes",
  "03",   "Impostos",                                      "Receitas Correntes",
  "04",   "Taxas",                                         "Receitas Correntes",
  "05",   "Contribuições",                                 "Receitas Correntes",
  "06",   "Contribuições Sociais",                         "Receitas Correntes",
  "07",   "Contribuições de Intervenção no Domínio Econ.", "Receitas Correntes",
  "08",   "Receita Patrimonial",                           "Receitas Correntes",
  "16",   "Receita Agropecuária",                          "Receitas Correntes",
  "17",   "Receita Industrial",                            "Receitas Correntes",
  "18",   "Receitas de Serviços",                          "Receitas Correntes",
  "24",   "Transferências Correntes",                      "Receitas Correntes",
  "32",   "Outras Receitas Correntes",                     "Receitas Correntes",
  "39",   "RECEITAS DE CAPITAL (II)",                      "Receitas de Capital",
  "40",   "Operações de Crédito",                          "Receitas de Capital",
  "43",   "Alienação de Bens",                             "Receitas de Capital",
  "47",   "Amortizações de Empréstimos",                   "Receitas de Capital",
  "48",   "Transferências de Capital",                     "Receitas de Capital",
  ...
  "80",   "OPERAÇÕES DE CRÉDITO - REFINANCIAMENTO (IV)",   "Refinanciamento",
...
estrutura_despesas <- tibble::tribble(
  ~Ordem, ~Item,                                              ~Grupo,
  "01",   "DESPESAS (Exceto Intra-Orçamentárias) (IX)",       "Despesas",
  "02",   "Despesas Correntes",                               "Despesas Correntes",
  "03",   "Pessoal e Encargos Sociais",                       "Despesas Correntes",
  "04",   "Juros e Encargos da Dívida",                       "Despesas Correntes",
  "05",   "Outras Despesas Correntes",                        "Despesas Correntes",
  "06",   "Transferência a Estados, DF e Municípios",         "Despesas Correntes",
  "07",   "Benefícios Previdenciários",                       "Despesas Correntes",
  ...
  "11",   "Inversões Financeiras",                            "Despesas de Capital",
  "12",   "Amortização da Dívida",                            "Despesas de Capital",
  "13",   "Reserva de Contingência",                          "Reserva",
  "14",   "DESPESAS INTRA-ORÇAMENTÁRIAS (X)",                 "Despesas Intra",
  "26",   "SUBTOTAL DAS DESPESAS (XI) = (IX + X)",            "Subtotais",
  "27",   "AMORTIZAÇÃO DA DÍVIDA - REFINANCIAMENTO (XII)",    "Refinanciamento",
...
#| code-fold: true
#| code-summary: "Ver critérios - Receitas"

# =============================================================================
# CRITÉRIOS RREO ANEXO 01 - RECEITAS
# =============================================================================
# 85 critérios para classificação das receitas orçamentárias
# =============================================================================
...
# =============================================================================
# CRITÉRIOS RREO ANEXO 01 - DESPESAS
# =============================================================================
# 34 critérios para classificação das despesas orçamentárias
# =============================================================================
...
#| code-fold: true
#| code-summary: "Ver código de aplicação"

# =============================================================================
# APLICAÇÃO DOS CRITÉRIOS
# =============================================================================

cat("📊 Processando critérios do RREO Anexo 01...\n")

# -----------------------------------------------------------------------------
# Aplicar critérios de Receitas
# -----------------------------------------------------------------------------
...
#| code-fold: true
#| code-summary: "Ver cálculo do resultado"
...
df_resultado <- tibble(
  Tipo = c("Receita Total", "Despesa Total", "Resultado Orçamentário"),
  Valor = c(receita_total, despesa_total, resultado_orcamentario)
)

datatable(
  df_resultado,
  caption = "RREO Anexo 01 - Resultado Orçamentário",
  rownames = FALSE,
  options = list(
    pageLength = -1,
    dom = 't'
...
notas <- tibble::tribble(
  ~Item, ~Descrição,
  "1", "Valores em R$ (reais)",
  "2", "Receitas intra-orçamentárias: categorias 7 e 8",
  "3", "Despesas intra-orçamentárias: modalidade de aplicação 91",
  "4", "Refinanciamento excluído dos subtotais principais",
  "5", "Item informação 5 = Receita Realizada",
  "6", "Item informação 25 = Despesas Pagas",
  "7", "Resultado = Receita - Despesa (positivo = superávit)"
)
...
Os principais itens de informação utilizados são:

**Receitas:** - **1**: Previsão Inicial - **2**: Previsão Atualizada - **5**: Receita Realizada

**Despesas:** - **9**: Dotação Inicial - **23**: Dotação Atualizada - **10**: Despesas Empenhadas - **18**: Despesas Liquidadas - **25**: Despesas Pagas
:::

::: callout-warning
### Refinanciamento da Dívida

As operações de refinanciamento são tratadas separadamente:

-   **Receitas de refinanciamento**: Naturezas que começam com 21110201, 21210201, 81110201, 81210201
-   **Despesas de refinanciamento**: Grupo 6 + Elementos 76/77 + Subfunções 841-846 + Fonte 443

Estas operações são incluídas apenas no "Subtotal com Refinanciamento".
:::

::: callout-tip
### Classificação da Despesa

A classificação de despesas utiliza múltiplos atributos:

-   **Categoria Econômica**: 3 (Corrente), 4 (Capital), 9 (Reserva)
-   **Grupo de Despesa**: 1-6
-   **Modalidade de Aplicação**: 91 indica intra-orçamentária
-   **Elemento de Despesa**: 76/77 para refinanciamento
:::

# 📋 RREO Anexo 02 - Despesas por Função e Subfunção
...
|--------------------------|----------------------------------------------|
| **Relatório** | RREO - Relatório Resumido da Execução Orçamentária |
| **Anexo** | Anexo 02 - Demonstrativo da Execução das Despesas por Função/Subfunção |
| **Base Legal** | LRF Art. 52, inciso II, alínea "c" |
| **Periodicidade** | Bimestral |
| **Grau de Dificuldade** | Fácil |

## 📁 Arquivos Necessários

| Objeto | Arquivo | Descrição |
|---------------------|-----------------------|----------------------------|
| `dados_despesa` | tg_despesa\_\*.xlsx | Despesas orçamentárias com classificação funcional |

## 📋 Atributos Utilizados

| Atributo | Descrição | Uso no Anexo |
|---------------------|-----------------------|-----------------------------|
| `funcao_governo_codigo` | Código da função | Agrupamento principal (01-28) |
| `funcao_governo_nome` | Nome da função | Descrição da função |
| `subfuncao_governo_codigo` | Código da subfunção | Detalhamento (xxx) |
| `subfuncao_governo_nome` | Nome da subfunção | Descrição da subfunção |
| `refinanciamento` | Indicador de refinanciamento | Separar refinanciamento da dívida |
| `item_informacao_codigo` | Item de informação | 23 = Dotação Atualizada |
| `saldo_r_item_informacao` | Valor do saldo | Valor monetário |

## 🔗 Vínculos com Outros Anexos

| Anexo    | Vínculo                                 |
|----------|-----------------------------------------|
| RREO A01 | Totalização das despesas                |
| RREO A06 | Despesas primárias por função           |
| RGF A01  | Despesa com pessoal (função específica) |

## 📐 Estrutura do Demonstrativo

### Classificação Funcional

A classificação funcional segrega as dotações orçamentárias em funções e subfunções, buscando responder à indagação "em que área" de ação governamental a despesa será realizada.

**Funções Principais:** - 01 - Legislativa - 02 - Judiciária - 03 - Essencial à Justiça - 04 - Administração - 06 - Segurança Pública - 08 - Assistência Social - 09 - Previdência Social - 10 - Saúde - 12 - Educação - 28 - Encargos Especiais

### Refinanciamento da Dívida

Segregação específica para operações de refinanciamento da dívida pública.
...
if (exists("despesas_funcao") && exists("despesas_refinanciamento")) {
  ...
  cat(strrep("=", 60), "\n")
  cat(sprintf("Total de Funções: %d\n", n_funcoes))
  ...
  cat("Dados não disponíveis para resumo\n")
}
```
...
| `despesas_funcao`          | Despesas agrupadas por função          |

| `valor_liquidado_total_a02` \| Total da dotação atualizada \|

## 📌 Observações Importantes

1.  **Item de Informação 23**: Dotação Atualizada (LOA + Créditos Adicionais)

2.  **Refinanciamento**: Segregação obrigatória para operações de refinanciamento da dívida pública mobiliária

3.  **Funções Típicas**:

    -   Funções 01-04: Poderes e Funções Essenciais
    -   Funções 05-28: Funções de Governo
    -   Função 28: Encargos Especiais (dívida, transferências)

4.  **Subfunções**: Podem ser combinadas com qualquer função (matricialidade)
...
# =============================================================================
# Estes critérios classificam as receitas correntes conforme a estrutura
# do Anexo 03 do RREO
# =============================================================================
...
criterios_rreo_A03_rcl_receitas_rcl <- list(
  ...
  # ---------------------------------------------------------------------------
  # Classificação conforme a origem da receita corrente
  ...
  # ---------------------------------------------------------------------------
  # DEDUÇÕES DAS RECEITAS (ITENS 10-16)
  # ---------------------------------------------------------------------------
  # Estes itens são deduzidos das receitas correntes para chegar à RCL
  # ---------------------------------------------------------------------------
  
  # TRANSFERÊNCIAS CONSTITUCIONAIS E LEGAIS (item 10)
  # FPE, FPM, FUNDEB, e outras transferências obrigatórias
  ...
  # Fonte 054 - Contribuição para Seguridade Social
  ...
  # CONTRIB. DO SERVIDOR PARA O PLANO DE PREVIDÊNCIA (item 12)
  # Fontes 055 e 056 - Contribuições Previdenciárias do Servidor
  ...
  # COMPENSAÇÃO FINANC. ENTRE REGIMES PREVIDÊNCIA (item 13)
  # Compensações entre RGPS e RPPS
  ...
  # CONTRIB. DOS MILITARES PARA O CUSTEIO DAS PENSÕES (item 14)
  # Contribuições específicas dos militares
  ...
  # CONTRIBUIÇÕES PARA PIS/PASEP (item 15)
  ...
  # DEDUÇÕES DAS RECEITAS (item 16)
  # Diversas deduções por fonte e natureza
...
cat("✅ Critérios de Receitas aplicados!\n")
```

### Critérios para Despesas (Deduções)
...
# =============================================================================
# Estes critérios classificam as despesas que são deduzidas da RCL
# =============================================================================
...
criterios_rreo_A03_rcl_despesas_rcl <- list(
  ...
  # ---------------------------------------------------------------------------
  # Ações orçamentárias de transferências obrigatórias
  # Modalidades de aplicação: 30, 31, 32, 35, 36, 40, 41, 42, 45, 46
  # Programa: 0903 (Transferências Constitucionais)
  # ---------------------------------------------------------------------------
...
cat("✅ Critérios de Despesas (Deduções) aplicados!\n")
```

### Critérios para RP Cancelados
...
# =============================================================================
# CRITÉRIOS PARA RP CANCELADOS - ANEXO 3 RCL
# =============================================================================
# RP cancelados de transferências constitucionais devem ser somados de volta
# =============================================================================
...
# Aplicar critérios de RP cancelados
aplicar_criterios(
  df = rreo_a03_rcl_despesas, 
...
cat("✅ Critérios de RP Cancelados aplicados!\n")
```
...
cat(sprintf("   ✓ Total de registros consolidados: %s\n", 
...
cat("\n📈 Calculando RCL dos últimos 12 meses...\n")
...
cat(sprintf("\n💉 Mínimo para Saúde (15%% RCL exercício): R$ %s\n",
...
cat(sprintf("📅 RCL (exercício %s): %s\n\n", 
            year(ym(mes_filtro)), 
            formatar_real(rcl_exercicio)))

# Mínimo Constitucional para Saúde
cat(sprintf("💉 Mínimo Saúde (15%% RCL exercício): %s\n", 
            formatar_real(minimo_saude)))

cat("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
```

### RCL Mensal (Últimos 12 meses)
...
Os seguintes objetos ficam disponíveis para uso em outros anexos:

| Objeto         | Descrição                                           |
|----------------|-----------------------------------------------------|
| `rcl`          | Dados detalhados por categoria e mês                |
| `rcl_12_meses` | RCL agregada por mês (últimos 12 meses)             |
| `minimo_saude` | Valor do mínimo constitucional para saúde (15% RCL) |

------------------------------------------------------------------------

## 📑 Anexos que Utilizam a RCL

A RCL calculada neste anexo é utilizada como **base de cálculo** para:

### RGF - Relatório de Gestão Fiscal

| Anexo | Descrição | Uso da RCL |
|------------------|--------------------------|----------------------------|
| **RGF A01** | Despesa com Pessoal | Limite de 60% (União) |
| **RGF A02** | Dívida Consolidada Líquida | Limite de endividamento |
| **RGF A03** | Garantias e Contragarantias | Limite de garantias |
| **RGF A04** | Operações de Crédito | Limite de operações |
...
| Anexo        | Descrição                     | Uso da RCL           |
|--------------|-------------------------------|----------------------|
| **RREO A12** | Receitas e Despesas com Saúde | Mínimo de 15% da RCL |

------------------------------------------------------------------------

## 💡 Observações Importantes

1.  **Base para LRF**: A RCL é fundamental para verificar cumprimento dos limites da Lei de Responsabilidade Fiscal

2.  **Cálculo**: RCL = Receitas Correntes - Transferências Constitucionais - Contribuições Previdenciárias + RP Cancelados

3.  **Período**: Utiliza-se a RCL acumulada dos últimos 12 meses (período móvel)

4.  **Mínimo Saúde**: 15% da RCL do **exercício corrente** (não dos últimos 12 meses)

5.  **Crítico**: Todos os anexos RGF dependem deste cálculo para verificação de limites
...
| Relatório           | RREO - Relatório Resumido da Execução Orçamentária    |
| Anexo               | Anexo 04                                              |
| Nome                | Demonstrativo das Receitas e Despesas Previdenciárias |
...
| Arquivo | Descrição | Filtros no TG |
|-------------------|-----------------------|-------------------------------|
| dados_receita | Receitas orçamentárias | Base compartilhada - UOs previdenciárias |
| dados_despesa | Despesas orçamentárias | Base compartilhada - Subfunções 272, 273, 274, 845, 846 |

## 📊 Atributos Utilizados

| Atributo | Tipo | Uso | Descrição |
|--------------------|----------------|----------------|----------------------|
| unidade_orcamentaria_codigo | character | TG / Critério | Código da UO (33904=INSS, 40904=FAT, 73901=FCDF) |
| natureza_receita_codigo_completo | character | TG / Critério | Código completo da natureza de receita |
| natureza_receita_nome | character | TG / Critério | Nome da natureza de receita |
| nre1_categoria_economica_codigo | character | TG / Critério | Categoria econômica (1=Corrente, 2=Capital, 7/8=Intra) |
| nre2_origem_receita_codigo_origem | character | TG / Critério | Origem da receita |
| fonte_recursos_codigo | character | TG / Critério | Código da fonte de recursos |
| fonte_recursos_detalhada_codigo | character | TG / Critério | Código detalhado da fonte |
| esfera_orcamentaria_codigo | character | TG / Critério | Esfera orçamentária (2=Seguridade) |
| funcao_governo_codigo | character | TG / Critério | Função de governo |
| subfuncao_governo_codigo | character | TG / Critério | Subfunção (272, 273, 274, 845, 846) |
| acao_governo_codigo | character | TG / Critério | Código da ação de governo |
| grupo_despesa_codigo_grupo | character | TG / Critério | Grupo de despesa (1=Pessoal, 3=ODC) |
| elemento_despesa_codigo | character | TG / Critério | Elemento de despesa (01, 03, 53-58, etc) |
| modalidade_aplicacao_codigo | character | TG / Critério | Modalidade (91=Intra) |
| item_informacao_codigo | character | TG / Critério | Tipo de info (5=Realizado, 25=Empenhado) |
| mes_lancamento | numeric | TG / R | Mês do lançamento (YYYYMM) |
| saldo_r_item_informacao | numeric | TG / R | Saldo do item de informação |

## 🔗 Vínculos com Outros Anexos

| Anexo Relacionado | Tipo de Vínculo | Descrição |
|-----------------------------|--------------------------|------------------|
| RREO Anexo 01 | Referência | Receitas orçamentárias base |
| RREO Anexo 02 | Referência | Despesas orçamentárias base |
| RGF Anexo 01 | Referência | Despesa com pessoal inclui inativos e pensionistas |
| RGF Anexo 02 | Referência | Passivo atuarial do RPPS e militares |
| RREO Anexo 14 | Consolidação | Projeção atuarial do RPPS |
...
-   **RPPS** - Regime Próprio de Previdência Social (Servidores Civis e Militares)
...
| 55902     | Fundo Nacional de Assistência Social        |
...
|----------------|--------------|-----------------------------------------------|
| UO excluída    | 73901        | Operações Oficiais de Crédito (vai para FCDF) |
| Fonte excluída | 054, 00, 000 | Fontes não previdenciárias                    |
...
criterios_rreo_A04_rpps_despesas_civil_militar_rpps <- list(
  ...
  `10  MILITARES - DESPESAS - Outros Benefícios Inativos` = list(
    criterio = "grupo_despesa_codigo_grupo %in% c(1) & elemento_despesa_codigo %notin% c('00', '01') & acao_governo_codigo %in% c('214H', '218K')"
  ),
  
  `11  MILITARES - DESPESAS - Compensação financeira entre RPPSU e os demais RPPS` = list(
...
## 📊 Resumo Consolidado Previdência
...
cat("\n")
cat(strrep("=", 70), "\n")
cat("RESUMO PREVIDENCIÁRIO - RREO ANEXO 04\n")
cat(strrep("=", 70), "\n\n")
...
if (exists("df_resultado_rgps")) {
  ...
  cat("📊 RGPS - Regime Geral de Previdência Social\n")
...
if (exists("df_resultado_rpps_civil")) {
  ...
  cat("👥 RPPS Civil - Servidores Civis\n")
...
if (exists("df_resultado_rpps_militar")) {
  ...
  cat("🎖️ RPPS Militar - Militares das Forças Armadas\n")
...
if (exists("df_resultado_fcdf")) {
  ...
  cat("🏛️ FCDF - Fundo Constitucional do DF\n")
...
## 📚 Estrutura do Demonstrativo - Receitas

### Receitas RGPS

| Ordem | Item | Critério Principal |
|----------------|----------------|----------------------------------------|
| 01 | Receitas Correntes - Empregadores, Trabalhadores | natureza 12100311-13, 1214\* |
| 02 | Receitas Correntes - Demais Contribuições | origem = 2, exceto item 01 |
| 03 | Outras Receitas - Compensação RPPS/RGPS | origem = 9 |
| 04 | Demais Receitas Correntes | categoria = 1, exceto origens 2 e 9 |
| 05 | Receitas de Capital - Alienações | categoria = 2, origem = 2 |
| 06 | Demais Receitas de Capital | categoria = 2, exceto origem 2 |
| 07 | Receitas INTRA-ORÇAMENTÁRIAS | categoria = 7 ou 8 |
...
## 📚 Estrutura do Demonstrativo - Despesas
...
| 02    | Pensões                          | elemento = 55, 56; grupo = 3       |
| 03    | Outros Benefícios                | elemento = 57, 58; grupo = 3       |
| 04    | Compensação RGPS para RPPS       | ação = 009W, 0531; grupo = 3       |
| 05    | Demais Despesas                  | grupo = 3, exceto itens anteriores |
| 06    | A Detalhar                       | elemento = 00; grupo = 3           |
| 07    | Despesas Previdenciárias (INTRA) | modalidade = 91; grupo = 3         |

### Despesas RPPS

| Ordem | Item | Critério Principal |
|----------------|----------------|----------------------------------------|
| 01 | CIVIS - A detalhar | subfunção 272-274/845, grupo=1, elemento=00 |
| 02 | CIVIS - Aposentadorias | subfunção 272-274/845, grupo=1, elemento=01 |
| 03 | CIVIS - Pensões | subfunção 272-274/845, grupo=1, elemento=03 |
| 04 | CIVIS - Outros Benefícios | subfunção 272-274/845-846, grupo=1 |
| 05-11 | MILITARES | ações 0179, 214H, 218K, 00QD, 00X3 |
...
Quando **negativo**, indica **déficit previdenciário** que deve ser coberto pelo Tesouro Nacional.

### Separação RGPS x RPPS

| Aspecto      | RGPS                   | RPPS                |
|--------------|------------------------|---------------------|
| Público      | Trabalhadores privados | Servidores públicos |
| Gestão       | INSS                   | Tesouro Nacional    |
| UO Principal | 33904                  | Diversas por órgão  |
| Elementos    | 53-58                  | 01, 03              |
| Subfunção    | 122 (INSS)             | 272, 273, 274       |

### Códigos de Ações Previdenciárias

| Código | Descrição | Regime |
|----|----|----|
| 0053 | Pagamento de Aposentadorias e Pensões - Civis | RPPS Civil |
| 0181 | Pagamento de Pessoal Inativo - Civis | RPPS Civil |
| 0179 | Pagamento de Pensões Militares | RPPS Militar |
| 214H | Pagamento de Inativos das Forças Armadas | RPPS Militar |
| 218K | Pagamento de Inativos - Civis do MD | RPPS Militar |
| 00X3 | Compensação Financeira entre RPPSU e demais RPPS | RPPS |
| 009W | Compensação Previdenciária RGPS/RPPS | RGPS |
| 00Q2, 00QN, 00NS | Ações FCDF | FCDF |
...
| Fonte 054 | Contribuições Sociais do RGPS - evitar duplicidade |
| Elementos 04, 07, 11, 13, 16, 39, 67, 96 | Não são benefícios previdenciários |
| Ação 09HB | Ação não previdenciária |
| Função 08, 10 | Assistência Social e Saúde - não é previdência |
...
| Objeto | Descrição |
|-------------------------------|-----------------------------------------|
| criterios_rreo_A04_rgps_receitas_rgps | Critérios receitas RGPS |
| criterios_rreo_A04_rgps_despesas_rgps | Critérios despesas RGPS |
| criterios_rreo_A04_rpps_receitas_civil_militar_rpps | Critérios receitas RPPS |
| criterios_rreo_A04_rpps_despesas_civil_militar_rpps | Critérios despesas RPPS |
| criterios_rreo_A04_rpps_receitas_fcdf | Critérios receitas FCDF |
| criterios_rreo_A04_rpps_despesas_fcdf | Critérios despesas FCDF |
| df_resultado_rgps | Resultado Previdenciário RGPS |
...
1.  **Período de Apuração**: O anexo apresenta valores do bimestre e acumulados no exercício.

2.  **Projeção Atuarial**: O RREO Anexo 14 apresenta a projeção atuarial do RPPS para os próximos 75 anos.

3.  **Meta Fiscal**: O resultado previdenciário impacta diretamente o Resultado Primário da União.

4.  **Passivo Atuarial**: O RGF Anexo 02 apresenta o passivo atuarial do RPPS e dos militares na DCL.

5.  **Receitas Intra-Orçamentárias**: Categoria 7 ou 8 representam transferências entre órgãos do mesmo ente.
...
|--------------------------|----------------------------------------------|
| **Relatório** | RREO - Relatório Resumido da Execução Orçamentária |
| **Anexo** | Anexo 06 - Demonstrativo dos Resultados Primário e Nominal |
...
| Objeto | Arquivo | Descrição |
|---------------------|-----------------------|----------------------------|
| `dados_receita` | tg_receita\_\*.xlsx | Receitas orçamentárias |
...
| `dados_despesa` | tg_despesa\_\*.xlsx | Despesas orçamentárias (subsídios CESEF) |
...
| `comparativo` | comparativo.xlsx | Valores oficiais RTN para ajuste metodológico |
...
# ===============================================================================
# CRITÉRIOS RREO ANEXO 6 - RESULTADO PRIMÁRIO (AJUSTADO)
# ===============================================================================
...
criterios_rreo_A06_receitas_A06 <- list(
  ...
  # RECEITAS PRIMÁRIAS CORRENTES (IV) - FÓRMULA
  `11  RECEITAS PRIMÁRIAS CORRENTES (IV) = (I - II - III)` = list(
    criterio = "{01 - 05 - 09}"
  ),
  ...
  `13  ___Operações de Crédito (VI)` = list(
    criterio = "
    nre1_categoria_economica_codigo == 2 & 
    nre2_origem_receita_codigo_origem == 1 & 
    item_informacao_codigo == 5
  ...
  `14  ___Amortização de Empréstimos (VII)` = list(
    criterio = "
    nre1_categoria_economica_codigo == 2 & 
    nre2_origem_receita_codigo_origem == 3 & 
    item_informacao_codigo == 5
  ...
  # RECEITAS DE ALIENAÇÃO DE INVESTIMENTOS PERMANENTES (IX) - Sub-subitem
  `16  ______Receitas de Alienação de Investimentos Permanentes (IX)` = list(
    criterio = "
  ...
  # OUTRAS ALIENAÇÕES DE BENS (Sub-subitem)
  `17  ______Outras Alienações de Bens` = list(
    criterio = "
    natureza_receita_codigo_completo == '22110011' & 
    item_informacao_codigo == 5
    "
  ...
  `18  ___Transferências de Capital` = list(
    criterio = "
    nre1_categoria_economica_codigo == 2 & 
    nre2_origem_receita_codigo_origem == 4 & 
    item_informacao_codigo == 5
  ...
  `20  ______Outras Receitas de Capital Não Primárias (X)` = list(
    criterio = "
    nre1_categoria_economica_codigo == 2 & 
  ...
  # RECEITAS PRIMÁRIAS DE CAPITAL (XI) - FÓRMULA
  `21  RECEITAS PRIMÁRIAS DE CAPITAL (XI) = (V - VI - VII - IX - X)` = list(
    criterio = "{12 - 13 - 14 - 16 - 20}"
  ),
  
  # RECEITA PRIMÁRIA TOTAL (XII) - FÓRMULA
  `22  RECEITA PRIMÁRIA TOTAL (XII) = (IV + XI)` = list(
    criterio = "{11 + 21}"
  )
)
...
criterios_rreo_A06_despesas_categoria_grupo_A06 <- list(
  ...
  # JUROS E ENCARGOS DA DÍVIDA (XIV) - DEDUÇÃO (Subitem)
  `25  ___Juros e Encargos da Dívida (XIV)` = list(
    criterio = "
    categoria_economica_despesa_codigo == 3 & 
    grupo_despesa_codigo_grupo == 2 & 
    !acao_governo_codigo %in% c('0021', '0029', '0030', '0031', '0061', '006A', '0427', 
  ...
  # DESPESAS PRIMÁRIAS CORRENTES (XV) - FÓRMULA
  `27  DESPESAS PRIMÁRIAS CORRENTES (XV) = (XIII - XIV)` = list(
    criterio = "{23 - 25}"
  ),
  ...
  # INVERSÕES FINANCEIRAS (Subitem)
  `30  ___Inversões Financeiras` = list(
    criterio = "
    categoria_economica_despesa_codigo == 4 & 
    grupo_despesa_codigo_grupo == 5 & 
    !acao_governo_codigo %in% c('0021', '0029', '0030', '0031', '0061', '006A', '0427', 
  ...
  # AMORTIZAÇÃO DA DÍVIDA (XX) - DEDUÇÃO (Subitem)
  `31  ___Amortização da Dívida (XX)` = list(
    criterio = "
    categoria_economica_despesa_codigo == 4 & 
    grupo_despesa_codigo_grupo == 6 & 
    !acao_governo_codigo %in% c('0021', '0029', '0030', '0031', '0061', '006A', '0427', 
...
criterios_rreo_A06_despesas_natureza_detalhada_A06 <- list(
  ...
  `32  ______Transferências Constitucionais e Legais` = list(
    criterio = "
  ...
  # DEMAIS DESPESAS CORRENTES - FÓRMULA
  `33  ______Demais Despesas Correntes` = list(
    criterio = "{26 - 32}"
  ),
  
  # CONCESSÃO DE EMPRÉSTIMOS E FINANCIAMENTOS (XVII)
  `34  ______Concessão de Empréstimos e Financiamentos (XVII)` = list(
    criterio = "
  ...
  # AQUISIÇÃO DE TÍTULOS DE CAPITAL JÁ INTEGRALIZADO (XVIII)
  `35  ______Aquisição de Títulos de Capital já Integralizado (XVIII)` = list(
    criterio = "
  ...
  # DEMAIS INVERSÕES FINANCEIRAS - FÓRMULA
  `36  ______Demais Inversões Financeiras` = list(
    criterio = "{30 - 34 - 35}"
  ),
  
  # COTAS DOS FUNDOS GARANTIDORES
  `38  ______Cotas dos Fundos Garantidores (metodologia CESEF - primário)` = list(
    criterio = "
...
# ===============================================================================
# 6. SUBSÍDIOS CESEF (dados_despesa) - Item 37
# ===============================================================================
criterios_rreo_A06_subsidios_cesef <- list(
  
  `37  ______Subsídios (metodologia CESEF - financeiro primário)` = list(
    criterio = "
    acao_governo_codigo %in% c('0021', '0029', '0030', '0031', '0061', '006A', '0427', 
                              '0534', '0A81', '0A84', '2130', '0062', '00DD', '00S5', 
                              '20GI', '006C', '00ED', '00EE', '00SG')
...
criterios_rreo_A06_emprestimos <- list(
  ...
  `38  ______Concessão de Empréstimos e Financiamentos` = list(
    criterio = "
    acao_governo_codigo %in% c('0021', '0029', '0030', '0031', '0061', '006A', '0427', 
                              '0534', '0A81', '0A84', '2130', '0062', '00DD', '00S5', 
                              '20GI', '006C', '00ED', '00EE', '00SG') & (
...
criterios_rreo_A06_ajuste <- list(
  ...
  # VPD - VARIAÇÕES MONETÁRIAS DA DÍVIDA CONTRATUAL
  `50 VPD - Variações Monetárias da Dívida Contratual` = list(
    criterio = "
    conta_contabil_numero %in% c('343110100', '343540100', '343550100')
    "
  ),

  # VPD - VARIAÇÕES CAMBIAIS DA DÍVIDA CONTRATUAL
  `51 VPD - Variações Cambiais da Dívida Contratual` = list(
    criterio = "
    conta_contabil_numero == '343210200'
    "
  ),

  # VPD - VARIAÇÕES MONETÁRIAS DA DÍVIDA MOBILIÁRIA
  `52 VPD - Variações Monetárias da Dívida Mobiliária` = list(
    criterio = "
  ...
  # VPD - VARIAÇÕES CAMBIAIS DA DÍVIDA MOBILIÁRIA
  `53 VPD - Variações Cambiais da Dívida Mobiliária` = list(
    criterio = "
  ...
  # VPD - ATUALIZAÇÃO MONETÁRIA NEGATIVA
  `54 VPD - Atualização Monetária Negativa` = list(
    criterio = "
  ...
  # VPD - INCORPORAÇÃO DE PASSIVOS
  `55 VPD - Incorporação de Passivos` = list(
    criterio = "
  ...
  # VPD - CONSTITUIÇÃO DE SUBVENÇÕES ECONÔMICAS - COPEC
  `57 VPD - Constituição de Subvenções Econômicas - COPEC` = list(
    criterio = "
  ...
  # VPD - OUTRAS VARIAÇÕES CAMBIAIS
  `58 VPD - Outras Variações Cambiais` = list(
    criterio = "
  ...
  # VPD - DESINCORPORAÇÃO DE ATIVOS
  `59 VPD - Desincorporação de Ativos` = list(
    criterio = "
  ...
  # VPD - REVERSÃO DE SUBVENÇÕES ECONÔMICAS - COPEC
  `60 VPD - Reversão de Subvenções Econômicas - COPEC` = list(
    criterio = "
    conta_contabil_numero %in% c('395010100', '395110100') &
    evento_codigo == '541559' &
    ug_executora_codigo == '170700'
  ...
  # VPA - ATUALIZAÇÃO MONETÁRIA POSITIVA
  `63 VPA - Atualização Monetária Positiva` = list(
    criterio = "
  ...
  # VPA - OUTRAS VARIAÇÕES MONETÁRIAS
  `64 VPA - Outras Variações Monetárias` = list(
    criterio = "
  ...
  # VPA - OUTRAS VARIAÇÕES CAMBIAIS
  `65 VPA - Outras Variações Cambiais` = list(
    criterio = "
  ...
  # VPA - GANHOS COM DESINCORPORAÇÃO DE PASSIVOS
  `66 VPA - Ganhos com Desincorporação de Passivos` = list(
    criterio = "
  ...
  `68 VPA - Outras Variações Patrimoniais Aumentativas` = list(
    criterio = "
  ...
  # VPA - REMUNERAÇÃO DOS DEPÓSITOS BANCÁRIOS
  `69 VPA - Remuneração dos Depósitos Bancários` = list(
    criterio = "
    conta_contabil_numero %in% c('445110100', '445210100') &
  ...
  # VPA - REVERSÃO DE PROVISÕES E DE AJUSTES PARA PERDAS
  `70 VPA - Reversão de Provisões e de Ajustes para Perdas` = list(
    criterio = "
  ...
  # VPA - OUTROS GANHOS COM INCORPORAÇÃO DE ATIVOS
  `71 VPA - Outros Ganhos com Incorporação de Ativos` = list(
    criterio = "
  ...
  # (-) CRÉDITO EMPENHADO EM LIQUIDAÇÃO (EXCETO CODIV)
  `72 Crédito Empenhado em Liquidação (exceto CODIV)` = list(
    criterio = "
  ...
  # APLICAÇÕES FINANCEIRAS - FECHAMENTO CÂMBIO
  `73 Aplicações Financeiras - Fechamento Câmbio` = list(
    criterio = "
  ...
  # PASSIVO DÍVIDA PÚBLICA REF FIES E MANTENEDORAS
  `75 Passivo Dívida Pública REF FIES e Mantenedoras` = list(
    criterio = "
  ...
  `76 Ajustes de Exercícios Anteriores` = list(
    criterio = "
  ...
  # Constituição de Ajuste para Perdas
  `77 Constituição de Ajuste para Perdas` = list(
    criterio = "
...
## 📈 Cálculo do Resultado Primário e Nominal
...
if (file.exists(arquivo_comparativo)) {
  ...
  warning(sprintf("Arquivo não encontrado: %s\nUsando ajuste = 0", arquivo_comparativo))
  0
}
}, error = function(e) {
...
# ------------------------------------------------------------------------------
# 2. FUNÇÃO AUXILIAR - Obter valor por ordem do rreo_a06_final
# ------------------------------------------------------------------------------
...
atual <- list(
dcl = if(exists("valor_dcl_num")) round(valor_dcl_num / 1000, 0) else 0,
...
# 5.1 Variação Saldo RPP (XV)
var_saldo_rpp <- atual$rp_processados - dez_2024$rp_processados

# 5.2 Receita Alienação Investimentos (XI)
...
# ------------------------------------------------------------------------------
# 6. OUTROS AJUSTES (XVIII) - Cálculo automático
# ------------------------------------------------------------------------------

# 6.1 VPDs (+) - Variações Patrimoniais Diminutivas
vpd_var_cambial_contratual <- obter_item(51)
vpd_var_cambial_mobiliaria <- obter_item(53)
vpd_resultado_neg_bacen <- 0  # Quando houver critério
vpd_constituicao_ajustes <- 9748641  # TODO: criar critério específico
...
# 6.2 VPAs (-) - Variações Patrimoniais Aumentativas
vpa_outras_var_cambiais <- obter_item(65)
vpa_resultado_pos_bacen <- 0  # Quando houver critério
...
# 6.4 Outros
desp_fin_impacto_primario <- 7801975  # TODO: criar critério específico
...
# ------------------------------------------------------------------------------
# 8. JUROS NOMINAIS (já calculados no chunk anterior)
# ------------------------------------------------------------------------------
juros_nominais <- if(exists("juros_ativos") && exists("juros_passivos")) {
round((juros_ativos - juros_passivos) / 1000, 0)
} else {
0
}

# ------------------------------------------------------------------------------
# 9. RESULTADO PRIMÁRIO - ABAIXO DA LINHA
# ------------------------------------------------------------------------------
rp_abaixo_linha <- rn_ajustado_abaixo - juros_nominais

# ------------------------------------------------------------------------------
# 10. RESULTADO NOMINAL - ACIMA DA LINHA (já calculado)
# ------------------------------------------------------------------------------
rn_acima_linha <- if(exists("resultado_nominal_final")) {
round(resultado_nominal_final / 1000, 0)
} else {
0
}
...
componente = c(
  "CÁLCULO DO RESULTADO NOMINAL - ABAIXO DA LINHA",
  "",
  "DCL Dezembro/2024 (a)",
  "DCL Atual (b)",
  ...
  "RECEITA ALIENAÇÃO INVEST. PERMANENTES (XI)",
...
# ------------------------------------------------------------------------------
# 13. TABELA RESULTADO PRIMÁRIO ABAIXO DA LINHA
# ------------------------------------------------------------------------------
tabela_rp_abaixo <- tibble(
componente = c(
  "CÁLCULO DO RESULTADO PRIMÁRIO - ABAIXO DA LINHA",
  "",
  "RESULTADO NOMINAL AJUSTADO - Abaixo da Linha (XIX)",
  "(-) Juros e Encargos Ativos (VII)",
  "(+) Juros e Encargos Passivos (VIII)",
  ...
  "",
  "RESULTADO PRIMÁRIO - Abaixo da Linha (XXIII) = XIX - (VII - VIII)"
),
valor_milhares = c(
  NA, NA,
  rn_ajustado_abaixo,
  if(exists("juros_ativos")) round(juros_ativos / 1000, 0) else 0,
  if(exists("juros_passivos")) round(juros_passivos / 1000, 0) else 0,
  juros_nominais,
  NA,
  rp_abaixo_linha
)
...
mutate(valor_milhares = ifelse(is.na(valor_milhares), "", 
                               format(round(valor_milhares, 0), big.mark = ".", decimal.mark = ","))) %>%
knitr::kable(col.names = c("Componente", "R$ milhares"), align = c("l", "r"),
             caption = "RREO Anexo 6 - Resultado Primário Abaixo da Linha")
...
cat(strrep("=", 70), "\n")
cat(sprintf("Resultado Primário - Acima da Linha:    %s\n", 
          if(exists("resultado_primario_final")) format(round(resultado_primario_final / 1000, 0), big.mark = ".", decimal.mark = ",") else "N/A"))
cat(sprintf("Resultado Primário - Abaixo da Linha:   %s\n", 
...
## RREO Anexo 6 - Resultado Primário e Nominal - Abaixo da Linha

### Metodologia Oficial

O cálculo "abaixo da linha" é baseado na variação da Dívida Consolidada Líquida (DCL) entre dezembro do exercício anterior e o mês atual, com ajustes específicos.

---

### Chunk 1: Configuração dos Valores de Fechamento do Exercício Anterior

```{r config-fechamento-exercicio}
#| echo: true

# ==============================================================================
# VALORES DE FECHAMENTO DO EXERCÍCIO ANTERIOR (DEZ/2024)
# ==============================================================================
# Fonte: Planilha "Anexo 06 - Resultados Primário.xlsx"
# Aba: "Anexo 6 RP e RN Novo (INTERNET)"
# 
# OPÇÃO 1: Informar valores manualmente (mais seguro para produção)
# OPÇÃO 2: Ler da planilha automaticamente
# ==============================================================================

# ------------------------------------------------------------------------------
# OPÇÃO 1: VALORES MANUAIS (DEZ/2024 - valores em R$ mil)
# ------------------------------------------------------------------------------
# Descomente e ajuste conforme necessário

valores_dez_2024 <- list(
  # DCL e componentes
  divida_consolidada       = 9967635161,   # Linha 74 - DÍVIDA CONSOLIDADA (X)
  deducoes                 = 2790356030,   # Linha 75 - DEDUÇÕES (XI)
  disponibilidade_caixa    = 1497520168,   # Linha 76 - Disponibilidade de Caixa
  disp_caixa_bruta         = 1627514230,   # Linha 77 - Disponibilidade Caixa Bruta
  rp_processados           = 129994061,    # Linha 78 - (-) Restos a Pagar Processados
  demais_haveres           = 1292835861,   # Linha 79 - Demais Haveres Financeiros
  dcl                      = 7177279131,   # Linha 80 - DCL = X - XI
  
  # Juros
  juros_ativos             = 326660712,    # Linha 57 - JUROS E ENCARGOS ATIVOS
  juros_passivos           = 1146101005    # Linha 58 - JUROS E ENCARGOS PASSIVOS
)

cat("✅ Valores de Dez/2024 configurados manualmente\n")
cat(sprintf("   DCL Dez/2024: R$ %s mil\n", format(valores_dez_2024$dcl, big.mark = ".", decimal.mark = ",")))
```

---

### Chunk 2: Leitura Automática da Planilha (Opcional)

```{r ler-valores-planilha-abaixo-linha}
#| echo: true
#| eval: false

# ==============================================================================
# OPÇÃO 2: LER VALORES DA PLANILHA AUTOMATICAMENTE
# ==============================================================================

# Buscar arquivo "Anexo 06 - Resultados Primário"
arquivo_anexo06 <- list.files(
  path = mes_pasta,
  pattern = "Anexo.*06.*Resultado.*Prim",
  full.names = TRUE,
  ignore.case = TRUE
)

if (length(arquivo_anexo06) > 0) {
  
  cat("📁 Arquivo encontrado:", basename(arquivo_anexo06[1]), "\n")
  
  # Ler aba específica
  aba_rp_rn <- "Anexo 6 RP e RN Novo (INTERNET)"
  
  # Função para ler célula específica
  ler_celula_anexo06 <- function(linha, coluna = 3) {
    tryCatch({
      # Ler região específica
      dados <- read_excel(
        arquivo_anexo06[1],
        sheet = aba_rp_rn,
        range = cellranger::cell_limits(c(linha, coluna), c(linha, coluna)),
        col_names = FALSE
      )
      as.numeric(dados[[1]])
  ...
  # Ler valores de Dez/2024 (coluna C = 3)
  valores_dez_2024 <- list(
    divida_consolidada    = ler_celula_anexo06(74, 3),
    deducoes              = ler_celula_anexo06(75, 3),
    disponibilidade_caixa = ler_celula_anexo06(76, 3),
    disp_caixa_bruta      = ler_celula_anexo06(77, 3),
    rp_processados        = ler_celula_anexo06(78, 3),
    demais_haveres        = ler_celula_anexo06(79, 3),
    dcl                   = ler_celula_anexo06(80, 3),
    juros_ativos          = ler_celula_anexo06(57, 3),
    juros_passivos        = ler_celula_anexo06(58, 3)
  )
  
  cat("✅ Valores de Dez/2024 lidos da planilha\n")
  
} else {
  cat("⚠️ Planilha Anexo 06 não encontrada. Usando valores manuais.\n")
}
```

---

### Chunk 3: Coletar Valores do Mês Atual (do Sistema)

```{r valores-mes-atual-abaixo-linha}
#| echo: true

# ==============================================================================
# VALORES DO MÊS ATUAL - COLETADOS DO SISTEMA
# ==============================================================================

# DCL do mês atual (calculada no RGF A02)
dcl_mes_atual <- if (exists("valor_dcl_num")) {
  valor_dcl_num
} else if (exists("valor_dcl_rgf")) {
  valor_dcl_rgf
} else {
  warning("DCL do mês atual não encontrada")
  NA_real_
}

# RP Processados do mês atual (do RGF A05 ou balancete)
rp_processados_atual <- if (exists("rgf_a05_executivo_formatada")) {
  rgf_a05_executivo_formatada %>%
    filter(linhas == "Total") %>%
    pull(rp_liquidados_nao_pagos_exercicios_anteriores) %>%
    sum(na.rm = TRUE) +
  rgf_a05_executivo_formatada %>%
    filter(linhas == "Total") %>%
    pull(rp_liquidados_nao_pagos_exercicio) %>%
    sum(na.rm = TRUE)
} else {
  NA_real_
}

# Juros do mês atual (calculados no chunk anterior - acima da linha)
juros_ativos_atual <- if (exists("juros_ativos")) {
  juros_ativos
} else {
  NA_real_
}

juros_passivos_atual <- if (exists("juros_passivos")) {
  juros_passivos
} else {
  NA_real_
}

cat("📊 Valores do mês atual (sistema):\n")
cat(sprintf("   DCL: R$ %s\n", formatar_numero(dcl_mes_atual)))
cat(sprintf("   RP Processados: R$ %s\n", formatar_numero(rp_processados_atual)))
cat(sprintf("   Juros Ativos: R$ %s\n", formatar_numero(juros_ativos_atual)))
cat(sprintf("   Juros Passivos: R$ %s\n", formatar_numero(juros_passivos_atual)))
```

---

### Chunk 4: Calcular Resultado Abaixo da Linha

```{r calcular-resultado-abaixo-linha}
#| echo: true

# ==============================================================================
# CÁLCULO DO RESULTADO PRIMÁRIO E NOMINAL - ABAIXO DA LINHA
# ==============================================================================
# Metodologia STN/CESEF
# ==============================================================================

# ------------------------------------------------------------------------------
# 1. RESULTADO NOMINAL - Abaixo da Linha (XIV) = DCL(Dez) - DCL(Atual)
# ------------------------------------------------------------------------------
resultado_nominal_abaixo_bruto <- valores_dez_2024$dcl*1000 - dcl_mes_atual

# ------------------------------------------------------------------------------
# 2. VARIAÇÃO SALDO RPP (XV) = RP_Processados(Atual) - RP_Processados(Dez)
# ------------------------------------------------------------------------------
variacao_saldo_rpp <- - (valores_linhas %>% filter(ordem == 25) %>% pull(valor )) - valores_dez_2024$rp_processados*1000

# ------------------------------------------------------------------------------
# 3. RECEITA DE ALIENAÇÃO DE INVESTIMENTOS PERMANENTES (XIX)
# ------------------------------------------------------------------------------
# Normalmente zero ou valor específico - pode ser informado manualmente
receita_alienacao_inv_perm <- 0

# ------------------------------------------------------------------------------
# 4. PASSIVOS RECONHECIDOS NA DC (XX)
# ------------------------------------------------------------------------------
# Extrair do rreo_a06_final se disponível
passivos_reconhecidos_dc <- (valores_linhas %>% filter(ordem == 10) %>% pull(valor ) - rreo_a06_final %>% filter(ordem %in% c ( 66)) %>% pull (valor) + rreo_a06_final %>% filter(ordem %in% c (55)) %>% pull (valor))

# ------------------------------------------------------------------------------
# 5. OUTROS AJUSTES (XVIII) - Calculados dos itens 89-120
# ------------------------------------------------------------------------------

# 5.1 Variações Patrimoniais Diminutivas (+)
# Itens: 51, 53, 93, 94, 58, 59
vpd_cambiais_contratual <- obter_valor(51)  # Var Cambiais Dívida Contratual
vpd_cambiais_mobiliaria <- obter_valor(53)  # Var Cambiais Dívida Mobiliária
vpd_ajuste_perdas <- obter_valor(77)        # Constituição Ajuste para Perdas
vpd_subvencoes <- obter_valor(57) + obter_valor(60)  # Provisão/Reversão Subvenções COPEC
vpd_outras_cambiais <- obter_valor(58)      # Outras Variações Cambiais
vpd_desincorp_ativos <- obter_valor(59)     # Desincorporação de Ativos

total_vpd_ajuste <- vpd_cambiais_contratual + vpd_cambiais_mobiliaria + 
                    vpd_ajuste_perdas + vpd_subvencoes + 
                    vpd_outras_cambiais + vpd_desincorp_ativos

# 5.2 Variações Patrimoniais Aumentativas (-)
# Itens: 65, 68, 71, 70
vpa_outras_cambiais <- obter_valor(65)      # Outras Variações Cambiais
vpa_outras_vpa <- obter_valor(68)           # Outras VPA
vpa_outros_ganhos <- obter_valor(71)        # Outros Ganhos Incorporação Ativos
vpa_reversao_provisoes <- obter_valor(70)   # Reversão de Provisões

total_vpa_ajuste <- vpa_outras_cambiais + vpa_outras_vpa + 
                    vpa_outros_ganhos + vpa_reversao_provisoes

# 5.3 Demais Ajustes
# Ajustes nos itens da DCL
variacao_precatorios <- 0  # Seria calculado da variação entre períodos
variacao_arrecadacao_recolher <- 0
variacao_aplic_fundos <- 0
juros_fundos_liquidos <- obter_valor(43) - obter_valor(44)  # Fundos: Ativos - Passivos
divida_fies <- obter_valor(75)

# Outros
variacao_fontes_classificar <- obter_valor(39)  # Recursos a Classificar
despesas_fin_impacto_primario <- 0  # Despesas Financeiras com Impacto Primário
demais_desp_primarias_extra <- 0
ajustes_exerc_anteriores <- obter_valor(76)

ajustes_dcl <- variacao_precatorios - variacao_arrecadacao_recolher - 
               variacao_aplic_fundos + juros_fundos_liquidos + divida_fies

outros_ajustes <- -variacao_fontes_classificar - despesas_fin_impacto_primario - 
                  demais_desp_primarias_extra - ajustes_exerc_anteriores

demais_ajustes <- ajustes_dcl + outros_ajustes

# Total Outros Ajustes (XVIII)
outros_ajustes_total <- total_vpd_ajuste - total_vpa_ajuste + demais_ajustes

# ------------------------------------------------------------------------------
# 6. RESULTADO NOMINAL AJUSTADO - Abaixo da Linha (XIX)
# ------------------------------------------------------------------------------
# XIX = XIV + XV - Alienação + Passivos DC + Outros Ajustes
resultado_nominal_ajustado <- resultado_nominal_abaixo_bruto + 
                              variacao_saldo_rpp - 
                              receita_alienacao_inv_perm + 
                              passivos_reconhecidos_dc + 
                              outros_ajustes_total

# ------------------------------------------------------------------------------
# 7. RESULTADO PRIMÁRIO - Abaixo da Linha (XXIII)
# ------------------------------------------------------------------------------
# XXIII = XIX - (Juros Ativos - Juros Passivos)
juros_liquidos_atual <- juros_ativos_atual - juros_passivos_atual

resultado_primario_abaixo_final <- resultado_nominal_ajustado - juros_liquidos_atual

# ------------------------------------------------------------------------------
# 8. DISCREPÂNCIA com Acima da Linha
# ------------------------------------------------------------------------------
discrepancia_rp <- if (exists("resultado_primario_final")) {
  resultado_primario_final - resultado_primario_abaixo_final
} else {
  NA_real_
}

discrepancia_rn <- if (exists("resultado_nominal_final")) {
  resultado_nominal_final - resultado_nominal_ajustado
} else {
  NA_real_
}

# RP atual
valores_linhas %>% filter(ordem == 25) %>% pull(valor )

# passivos reconhecidos
valores_linhas %>% filter(ordem == 10) %>% pull(valor ) - rreo_a06_final %>% filter(ordem %in% c ( 66)) %>% pull (valor) + rreo_a06_final %>% filter(ordem %in% c (55)) %>% pull (valor)


#vpd
# 54 está zerado
rreo_a06_final %>% filter(ordem == 51) %>% pull(valor ) + rreo_a06_final %>% filter(ordem %in% c ( 53)) %>% pull (valor)  + rreo_a06_final %>% filter(ordem == 77) %>% pull(valor ) + rreo_a06_final %>% filter(ordem %in% c ( 60)) %>% pull (valor) + rreo_a06_final %>% filter(ordem %in% c (58)) %>% pull (valor) + rreo_a06_final %>% filter(ordem %in% c (59)) %>% pull (valor) 

#VPA
# 67 está zerado
rreo_a06_final %>% filter(ordem == 65) %>% pull(valor ) + rreo_a06_final %>% filter(ordem %in% c ( 70)) %>% pull (valor)  + rreo_a06_final %>% filter(ordem == 71) %>% pull(valor ) + rreo_a06_final %>% filter(ordem %in% c ( 68)) %>% pull (valor) 


# Aplicações de Fundos Diversos Junto ao Setor Privado
# dez/24 40841099,2214191
valores_linhas %>% filter(ordem == 14) %>% pull(valor ) + valores_linhas %>% filter(ordem == 15) %>% pull(valor ) + valores_linhas %>% filter(ordem == 16) %>% pull(valor ) - valores_linhas %>% filter(ordem == 13) %>% pull(valor )

rreo_a06_final %>% filter(ordem == 73) %>% pull(valor )

#precatorios
#  dez/2024
valores_linhas %>% filter(ordem == "08") %>% pull(valor ) + valores_linhas %>% filter(ordem == 38) %>% pull(valor )
...
### Chunk 5: Exibir Resultados

```{r exibir-resultado-abaixo-linha}
#| echo: false

# ==============================================================================
# EXIBIR RESULTADOS - ABAIXO DA LINHA
# ==============================================================================

cat("\n")
cat("=" %>% strrep(80), "\n")
cat("📊 RESULTADO PRIMÁRIO E NOMINAL - ABAIXO DA LINHA\n")
cat("=" %>% strrep(80), "\n\n")

cat("▶ VARIAÇÃO DA DÍVIDA CONSOLIDADA LÍQUIDA:\n")
cat(sprintf("   DCL Dez/2024:               R$ %s\n", formatar_numero(valores_dez_2024$dcl)))
cat(sprintf("   DCL Mês Atual:              R$ %s\n", formatar_numero(dcl_mes_atual)))
cat(sprintf("   Variação (XIV):             R$ %s\n\n", formatar_numero(resultado_nominal_abaixo_bruto)))

cat("▶ AJUSTES:\n")
cat(sprintf("   Variação Saldo RPP (XV):    R$ %s\n", formatar_numero(variacao_saldo_rpp)))
cat(sprintf("   (-) Alienação Inv Perm:     R$ %s\n", formatar_numero(receita_alienacao_inv_perm)))
cat(sprintf("   (+) Passivos Reconh DC:     R$ %s\n", formatar_numero(passivos_reconhecidos_dc)))
cat(sprintf("   Outros Ajustes (XVIII):     R$ %s\n\n", formatar_numero(outros_ajustes_total)))

cat("-" %>% strrep(80), "\n")
cat(sprintf("📌 RESULTADO NOMINAL AJUSTADO (XIX):    R$ %s\n", formatar_numero(resultado_nominal_ajustado)))
cat("-" %>% strrep(80), "\n\n")

cat("▶ JUROS NOMINAIS:\n")
cat(sprintf("   Juros Ativos:               R$ %s\n", formatar_numero(juros_ativos_atual)))
cat(sprintf("   Juros Passivos:             R$ %s\n", formatar_numero(juros_passivos_atual)))
cat(sprintf("   Juros Líquidos:             R$ %s\n\n", formatar_numero(juros_liquidos_atual)))

cat("=" %>% strrep(80), "\n")
cat(sprintf("📌 RESULTADO PRIMÁRIO - ABAIXO DA LINHA (XXIII): R$ %s\n", 
            formatar_numero(resultado_primario_abaixo_final)))
cat("=" %>% strrep(80), "\n\n")

if (!is.na(discrepancia_rp)) {
  cat("▶ CONCILIAÇÃO ACIMA x ABAIXO DA LINHA:\n")
  cat(sprintf("   RP Acima da Linha:          R$ %s\n", formatar_numero(resultado_primario_final)))
  cat(sprintf("   RP Abaixo da Linha:         R$ %s\n", formatar_numero(resultado_primario_abaixo_final)))
  cat(sprintf("   DISCREPÂNCIA:               R$ %s\n", formatar_numero(discrepancia_rp)))
}
```

---

### Chunk 6: Tabela Detalhada

```{r tabela-abaixo-linha}
#| echo: false
#| column: page

# ==============================================================================
# TABELA DETALHADA - ABAIXO DA LINHA
# ==============================================================================

tabela_abaixo_linha <- tibble(
  componente = c(
    "CÁLCULO DO RESULTADO NOMINAL",
    "DÍVIDA CONSOLIDADA (X)",
    "  Dez/2024",
    "  Mês Atual",
    "DEDUÇÕES (XI)",
    "  Dez/2024",
    ...
    "  Mês Atual",
    "",
    "RESULTADO NOMINAL - Abaixo Linha Bruto (XIV) = XIIIa - XIIIb",
    "",
    "VARIAÇÃO SALDO RPP (XV) = XIIb - XIIa",
    "RECEITA ALIENAÇÃO INV PERMANENTES (XIX)",
    "PASSIVOS RECONHECIDOS NA DC (XX)",
    "",
    "OUTROS AJUSTES (XVIII)",
    ...
    "  (+) Demais Ajustes",
    "",
    "RESULTADO NOMINAL AJUSTADO (XIX) = XIV + XV - XIX + XX + XVIII",
    "",
    "JUROS NOMINAIS",
    "  Juros e Encargos Ativos (VII)",
    "  Juros e Encargos Passivos (VIII)",
    "  Juros Líquidos = VII - VIII",
    "",
    "RESULTADO PRIMÁRIO - Abaixo da Linha (XXIII) = XIX - Juros Líquidos",
    "",
    "CONCILIAÇÃO",
    "  RP Acima da Linha",
    "  RP Abaixo da Linha",
  ...
  dez_2024 = c(
    NA, NA, valores_dez_2024$divida_consolidada, NA,
    NA, valores_dez_2024$deducoes, NA,
    NA, valores_dez_2024$dcl, NA, NA,
    NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA,
    valores_dez_2024$juros_ativos, valores_dez_2024$juros_passivos, NA, NA, NA, NA, NA, NA, NA, NA
  ),
  mes_atual = c(
    NA, NA, NA, dcl_mes_atual,
    NA, NA, NA,  # Deduções mês atual seria necessário calcular
    NA, NA, dcl_mes_atual, NA,
    resultado_nominal_abaixo_bruto, NA,
    variacao_saldo_rpp, receita_alienacao_inv_perm, passivos_reconhecidos_dc, NA,
    outros_ajustes_total, total_vpd_ajuste, total_vpa_ajuste, demais_ajustes, NA,
    resultado_nominal_ajustado, NA, NA,
    juros_ativos_atual, juros_passivos_atual, juros_liquidos_atual, NA,
    resultado_primario_abaixo_final, NA, NA,
    if(exists("resultado_primario_final")) resultado_primario_final else NA,
    resultado_primario_abaixo_final,
    discrepancia_rp
  )
)
...
datatable(
  tabela_abaixo_linha %>%
    mutate(
      dez_2024 = ifelse(is.na(dez_2024), "", 
                        format(round(dez_2024/1000, 0), big.mark = ".", decimal.mark = ",")),
      mes_atual = ifelse(is.na(mes_atual), "", 
                         format(round(mes_atual/1000, 0), big.mark = ".", decimal.mark = ","))
    ),
  caption = "RREO Anexo 6 - Resultado Primário e Nominal - Abaixo da Linha (R$ mil)",
  colnames = c("Componente", "Dez/2024", "Mês Atual"),
  options = list(pageLength = 40, dom = 't',
                 columnDefs = list(list(className = 'dt-right', targets = c(1, 2)))),
  rownames = FALSE
) %>%
  formatStyle('componente', fontWeight = styleEqual(
    c("RESULTADO NOMINAL - Abaixo Linha Bruto (XIV) = XIIIa - XIIIb",
      "RESULTADO NOMINAL AJUSTADO (XIX) = XIV + XV - XIX + XX + XVIII",
      "RESULTADO PRIMÁRIO - Abaixo da Linha (XXIII) = XIX - Juros Líquidos",
      "  DISCREPÂNCIA ESTATÍSTICA"),
    rep("bold", 4)
...
| Componente | Fórmula |
|------------|---------|
| **Resultado Nominal Bruto (XIV)** | DCL(Dez/2024) - DCL(Mês Atual) |
| **Variação Saldo RPP (XV)** | RP_Proc(Atual) - RP_Proc(Dez/2024) |
| **Resultado Nominal Ajustado (XIX)** | XIV + XV - Alienação + Passivos DC + Outros Ajustes |
| **Resultado Primário Abaixo (XXIII)** | XIX - (Juros Ativos - Juros Passivos) |

## Observações

1. **Valores de Dez/2024**: Podem ser informados manualmente ou lidos da planilha oficial
2. **DCL Mês Atual**: Calculada no RGF A02
3. **Juros**: Calculados no chunk "acima da linha" (itens 41-44 e 50-61)
4. **Discrepância**: Diferença entre cálculo "acima" e "abaixo" da linha
...
| Objeto | Descrição |
|-------------------------------|-----------------------------------------|
| `criterios_rreo_A06_receitas_A06` | Critérios para receitas primárias |
| `criterios_rreo_A06_despesas_categoria_grupo_A06` | Critérios para despesas por categoria/grupo |
| `criterios_rreo_A06_despesas_natureza_detalhada_A06` | Critérios para despesas por natureza detalhada |
| `criterios_rreo_A06_disponibilidades_A06` | Critérios para disponibilidades |
| `criterios_rreo_A06_juros_ativos_passivos_A06` | Critérios para juros ativos e passivos |
| `criterios_rreo_A06_subsidios_cesef` | Critérios para subsídios CESEF |
| `criterios_rreo_A06_ajuste` | Critérios para ajustes VPA/VPD |
| `rreo_a06_final` | Dados consolidados de todos os critérios |
| `valor_resultado_primario` | Resultado primário final |
| `valor_resultado_nominal` | Resultado nominal final |
| `valor_rp_abaixo_linha` | Resultado primário abaixo da linha |
| `valor_rn_ajustado_abaixo` | Resultado nominal ajustado abaixo da linha |
| `valor_discrepancia` | Discrepância entre acima e abaixo da linha |
...
|--------------------------|----------------------------------------------|
| **Relatório** | RREO - Relatório Resumido da Execução Orçamentária |
| **Anexo** | Anexo 07 - Demonstrativo dos Restos a Pagar por Poder e Órgão |
...
| Objeto              | Arquivo          | Descrição                             |
|---------------------|-----------------------|----------------------------|
| `dados_rp_anexo_07` | rreo_a07_rp.xlsx | Restos a pagar por órgão e modalidade |
...
| `orgao_uge_orgao_maximo_codigo` | Código do órgão máximo | Agrupamento por órgão |
| `orgao_uge_orgao_maximo_nome` | Nome do órgão máximo | Descrição do órgão |
| `item_informacao_codigo` | Item de informação | Tipo de RP (processado/não processado) |
| `item_informacao_nome` | Nome do item | Descrição do tipo de RP |
| `mes_lancamento` | Mês de lançamento | Filtro do período |
| `saldo_r_item_informacao` | Valor do saldo | Valor monetário |
...
| Anexo    | Vínculo                               |
|----------|---------------------------------------|
| RREO A01 | Despesas orçamentárias base           |
| RREO A06 | RP para cálculo do resultado primário |
...
**Por Situação:** - **RP Processados**: Despesas empenhadas e liquidadas, pendentes de pagamento - **RP Não Processados**: Despesas empenhadas mas não liquidadas

**Por Modalidade:** - **Intragovernamental**: Operações entre órgãos do mesmo ente - **Exceto Intragovernamental**: Demais operações

**Movimentação:** - Inscritos em Exercícios Anteriores - Inscritos em 31/12 do Exercício Anterior - Pagos - Cancelados - Saldo
...
# ==============================================================================
# DEFINIÇÃO DOS AGRUPAMENTOS PARA O ANEXO 07
# ==============================================================================

# Agrupamento por tipo de modalidade e órgão
...
# Agrupamento simplificado por órgão
...
### Tabela Principal - RP por Item de Informação
...
| **Base Legal** | LRF Art. 52, § 2º, inciso III; CF Art. 212 |
| **Periodicidade** | Bimestral |
| **Grau de Dificuldade** | Difícil |
...
| Objeto | Arquivo | Descrição |
|---------------------|-----------------------|----------------------------|
| `dados_receita` | tg_receita\_\*.xlsx | Receitas orçamentárias (impostos) |
| `dados_despesa` | tg_despesa\_\*.xlsx | Despesas orçamentárias com educação |
| `dados_rp_anexo_08` | rreo_a08_rp.xlsx | Restos a pagar de educação |

## 📋 Atributos Utilizados

| Atributo | Descrição | Uso no Anexo |
|---------------------|-----------------------|-----------------------------|
| `natureza_receita_codigo_completo` | Código completo da natureza | Identificar impostos (111*, 711*) |
| `fonte_recursos_codigo` | Código da fonte de recursos | Identificar fontes MDE (000, 008, 035, 133, 134, 213, 242) |
| `iduso_codigo` | Identificador de uso | 8 = MDE |
| `acao_governo_codigo` | Código da ação | 00SB, 0E36 = FUNDEB; 0312 = FCDF |
| `elemento_despesa_codigo` | Elemento de despesa | 01, 03, 59 = pessoal |
| `subfuncao_governo_codigo` | Subfunção | 363, 364, 368 = ensino |
| `plano_orcamentario_codigo_po` | Plano orçamentário | 0001 = VAAF; 0002/0003 = VAAT |
...
| RREO A02 | Despesas orçamentárias base                     |
| RGF A01  | Despesa com pessoal inclui inativos da educação |
...
-   **Transferências Constitucionais**: FPE, FPM, IPI, FUNDEB, IOF, ITR

### Despesas MDE

1.  **Complementação da União ao FUNDEB**: Ações 00SB, 0E36
2.  **Educação Básica**: Subfunção 368, IDUSO 8
3.  **Ensino Superior**: Subfunção 364, IDUSO 8
4.  **Ensino Profissional**: Subfunção 363, IDUSO 8
5.  **Outras**: Demais subfunções, IDUSO 8
6.  **Salário-Educação**: Fontes 133, 134, 213, 008, 035, 212
7.  **FCDF**: Ação 0312
8.  **Royalties Pré-Sal**: Fonte 242
...
# ==============================================================================
# CRITÉRIOS RREO ANEXO 08 - MDE (Manutenção e Desenvolvimento do Ensino)
# ==============================================================================
...
criterios_rreo_A08_mde_despesas <- list(
  ...
  # DESPESAS CUSTEADAS COM RECEITAS DE ROYALTIES DE EXPLORAÇÃO DO PRÉ-SAL
...
if (exists("receitas_base_rli") && exists("rli_transf")) {
  ...
  cat("Dados não disponíveis para cálculo da RLI\n")
...
if (exists("rli_mensal") && exists("mes_filtro")) {
  ...
  cat(paste("📅 RLI para", mes_filtro, ":", 
            format(round(rli_atual, 2), big.mark = ".", decimal.mark = ",")))
} else {
  rli_atual <- 0
  cat("RLI não calculada - dados não disponíveis\n")
}
```
...
if (exists("dados_despesa") && exists("mes_filtro")) {
  ...
  cat(paste("💰 Despesas MDE Principais:", 
            format(round(despesa_mde$valor, 2), big.mark = ".", decimal.mark = ",")))
} else {
  despesa_mde <- tibble(valor = 0)
  cat("Despesas MDE não calculadas - dados não disponíveis\n")
}
```
...
# Complementação FUNDEB (100% da complementação da União)
if (exists("dados_despesa") && exists("mes_filtro")) {
  ...
  cat(paste("🎯 Complementação FUNDEB (100%):", 
            format(round(complementacao_fundeb, 2), big.mark = ".", decimal.mark = ",")))
} else {
  complementacao_fundeb <- 0
  cat("Complementação FUNDEB não calculada - dados não disponíveis\n")
}
```
...
if (exists("dados_rp_anexo_08") && exists("mes_filtro")) {
  ...
  cat(paste("📋 Total RP MDE:", 
            format(round(despesa_mde_rp, 2), big.mark = ".", decimal.mark = ",")))
} else {
  despesa_mde_rp <- 0
  cat("RP MDE não calculado - dados não disponíveis\n")
}
```
...
# ==============================================================================
# CÁLCULO DO PERCENTUAL DE APLICAÇÃO EM MDE
# Mínimo constitucional: 18% da RLI
# ==============================================================================
...
resumo_aplicacao <- tibble(
  ...
  Status = c(
    ...
    "✅ Consolidado",
    "✅ Base",
    ifelse(percentual_aplicacao >= 100, "✅ Atendido", "❌ Não Atendido")
  )
)
...
cat("\n")
cat(strrep("=", 70), "\n")
cat("RESULTADO FINAL - APLICAÇÃO EM MDE\n")
cat(strrep("=", 70), "\n")
...
cat(sprintf("Situação: %s ao mínimo constitucional\n", 
            ifelse(percentual_aplicacao >= 100, "✅ ATENDE", "❌ NÃO ATENDE")))
cat(strrep("=", 70), "\n")
...
| Objeto | Descrição |
|-------------------------------|-----------------------------------------|
| `criterios_rreo_A08_mde_despesas` | Critérios para classificação das despesas MDE |
| `receitas_base_rli` | Receitas de impostos base para RLI |
| `rli_transf` | Transferências constitucionais |
| `rli_mensal` | RLI mensal calculada |
| `valor_rli_atual` | RLI do período de referência |
| `valor_despesa_mde` | Despesas MDE principais |
| `valor_complementacao_fundeb` | Complementação FUNDEB |
| `valor_total_mde` | Total de despesas MDE |
| `valor_percentual_mde` | Percentual de aplicação em MDE |

## 📌 Observações Importantes

1.  **Mínimo Constitucional**: A União deve aplicar no mínimo 18% da RLI em MDE (Art. 212 CF)

2.  **Complementação FUNDEB**:
...
3.  **Fontes Específicas**:
    ...
    -   133/134/213: Salário-Educação
    -   242: Royalties Pré-Sal
    -   000/050: Recursos Ordinários

4.  **Exclusões**:

    -   Elementos 01, 03, 59: Pessoal (não computado na base principal)
    -   Ações 00SB, 0E36: FUNDEB (tratamento especial)

# 📋 RREO Anexo 09 - Regra de Ouro
...
|--------------------------|----------------------------------------------|
| **Relatório** | RREO - Relatório Resumido da Execução Orçamentária |
| **Anexo** | Anexo 09 - Demonstrativo das Receitas de Operações de Crédito e Despesas de Capital |
| **Base Legal** | CF Art. 167, inciso III; LRF Art. 12, § 2º |
| **Periodicidade** | Bimestral |
| **Grau de Dificuldade** | Médio |
...
| Objeto          | Arquivo                | Descrição                        |
|---------------------|-----------------------|----------------------------|
| `dados_receita` | tg_receita\_\*.xlsx    | Receitas de operações de crédito |
| `dados_despesa` | tg_despesa\_\*.xlsx    | Despesas de capital              |
| `balancete`     | rgf_a02_balancete.xlsx | Saldo da subconta da dívida      |

## 📋 Atributos Utilizados

| Atributo | Descrição | Uso no Anexo |
|---------------------|-----------------------|-----------------------------|
| `natureza_receita_codigo_completo` | Código da natureza | Identificar operações de crédito (21*, 81*) |
| `categoria_economica_despesa_codigo` | Categoria da despesa | 4 = Capital |
| `conta_contabil_numero` | Número da conta contábil | 111110401 = Subconta da dívida |
| `item_informacao_codigo` | Item de informação | 5 = Receita realizada, 25 = Despesa liquidada |
| `mes_lancamento` | Mês de lançamento | Filtro do período |
| `saldo_r_item_informacao` | Valor do saldo | Valor monetário |
| `saldo_r_conta_contabil` | Saldo contábil | Saldo da subconta |

## 🔗 Vínculos com Outros Anexos

| Anexo    | Vínculo                           |
|----------|-----------------------------------|
| RREO A01 | Receitas e despesas orçamentárias |
| RREO A04 | Operações de crédito detalhadas   |
...
A "Regra de Ouro" estabelece que as receitas de operações de crédito não podem exceder o montante das despesas de capital, ressalvadas as autorizadas mediante créditos suplementares ou especiais com finalidade precisa, aprovados pelo Poder Legislativo por maioria absoluta.

**Fórmula:**

```         
Regra de Ouro = Despesas de Capital - (Receitas de Op. Crédito - Variação Subconta Dívida)

Se Regra de Ouro ≤ 0: CUMPRIDA
Se Regra de Ouro > 0: NÃO CUMPRIDA
```

### Componentes

1.  **Receitas de Operações de Crédito (I)**: Naturezas 21\* e 81\*
2.  **Variação da Subconta da Dívida (II)**: Conta 111110401
3.  **Receitas Líquidas (III = I - II)**
4.  **Despesas de Capital (IV)**: Categoria econômica 4
5.  **Resultado (IV - III)**
...
# ==============================================================================
# PARÂMETROS MANUAIS - AJUSTAR ANUALMENTE
# ==============================================================================

# Saldo inicial da subconta da dívida (abertura do exercício)
# Este valor deve ser atualizado no início de cada exercício
saldo_inicial_divida <- 731471695519.71  # Saldo abertura 2025 (000/2025)

cat(sprintf("📅 Saldo Inicial da Subconta da Dívida: R$ %s\n",
...
if (!exists("dados_receita")) {
  ...
  cat(sprintf("💰 Receitas de Operações de Crédito (I): R$ %s\n",
...
### 3. Variação da Subconta da Dívida
...
if (!exists("balancete")) {
  ...
  if (exists("rgf_a02_balancete")) {
    ...
    variacao_divida <- 0
    cat("⚠️ Dados do balancete não disponíveis\n")
  }
}
...
cat(strrep("=", 70), "\n")
cat(sprintf("Período de Referência: %s\n", mes_filtro))
cat(strrep("-", 70), "\n")
cat(sprintf("Receitas de Operações de Crédito (I):  R$ %s\n",
            format(round(receitas_op_credito, 2), big.mark = ".", decimal.mark = ",")))
cat(sprintf("(-) Variação Subconta da Dívida (II):  R$ %s\n",
            format(round(variacao_divida, 2), big.mark = ".", decimal.mark = ",")))
cat(sprintf("= Receitas Líquidas (III):             R$ %s\n",
...
cat(sprintf("STATUS: %s %s\n",
            ifelse(status_regra_ouro == "CUMPRIDA", "✅", "❌"),
            status_regra_ouro))
cat(strrep("=", 70), "\n")
...
| `receitas_op_credito`  | Receitas de operações de crédito |
| `despesas_capital`     | Despesas de capital liquidadas   |
| `saldo_atual_divida`   | Saldo atual da subconta          |
| `variacao_divida`      | Variação da subconta             |
| `receitas_liquidas`    | Receitas menos variação          |
| `regra_ouro`           | Resultado do cálculo             |
| `status_regra_ouro`    | CUMPRIDA ou NÃO CUMPRIDA         |
| `resultado_regra_ouro` | Tabela resumo                    |

## 📌 Observações Importantes

1.  **Base Constitucional**: Art. 167, III da CF/88 - vedação de operações de crédito que excedam despesas de capital
...
3.  **Subconta da Dívida (111110401)**:

    -   Registra recursos vinculados ao refinanciamento
    -   Variação positiva = aumento de recursos
    -   Variação negativa = utilização de recursos

4.  **Naturezas de Receita**:

    -   21\*: Operações de Crédito Internas e Externas
    -   81\*: Operações de Crédito Intraorçamentárias

5.  **Atualização Anual**:

    -   O saldo inicial da subconta deve ser atualizado no início de cada exercício
...
# 📋 RREO Tabela 01 - Receitas e Despesas Orçamentárias
...
|--------------------------|----------------------------------------------|
| **Relatório** | RREO - Relatório Resumido da Execução Orçamentária |
| **Demonstrativo** | Tabela 01 - Balanço Orçamentário da Seguridade Social |
...
|-----------------|---------------------|------------------------|
| `dados_receita` | tg_receita\_\*.xlsx | Receitas orçamentárias |
| `dados_despesa` | tg_despesa\_\*.xlsx | Despesas orçamentárias |

## 📋 Atributos Utilizados

| Atributo | Descrição | Uso na Tabela |
|--------------------|----------------------|------------------------------|
| `nre1_categoria_economica_codigo` | Categoria econômica receita | 1=Corrente, 2=Capital, 7/8=Intra |
| `nre2_origem_receita_codigo_origem` | Origem da receita | 1-9 conforme classificação |
| `grupo_despesa_codigo_grupo` | Grupo de despesa | 1-6, 9 |
| `esfera_orcamentaria_codigo` | Esfera orçamentária | 2=Seguridade Social |
| `item_informacao_codigo` | Item de informação | 5=Realizado, 25=Empenhado |
| `unidade_orcamentaria_codigo` | UO | 33904, 40904 etc = Previdência |
| `modalidade_aplicacao_codigo` | Modalidade | 30-45 = Transferências |

## 🔗 Vínculos com Outros Anexos

| Anexo    | Vínculo                     |
|----------|-----------------------------|
| RREO A01 | Receitas orçamentárias base |
| RREO A02 | Despesas orçamentárias base |
| RREO A04 | Detalhamento previdenciário |
...
-   **Receitas Correntes** (Categoria 1): Tributária, Contribuições, Patrimonial, etc.
-   **Receitas de Capital** (Categoria 2): Operações de Crédito, Alienações, etc.
-   **Receitas Intra-Orçamentárias** (Categorias 7 e 8)

### Despesas por Grupo

-   **Despesas Correntes**: Pessoal, Juros, ODC
-   **Despesas de Capital**: Investimentos, Inversões, Amortização
...
# ==============================================================================
# CRITÉRIOS DA TABELA 01 - RECEITAS ORÇAMENTÁRIAS
# ==============================================================================
...
criterios_rreo_T01_receitas <- list(
  ...
  # Receita de Contribuições (Origem 2)
  `02  Receita de Contribuições` = list(
    criterio = "
  ...
  # Transferências Correntes (Origem 7)
  `07  Transferências Correntes` = list(
    criterio = "
    nre1_categoria_economica_codigo == 1 & nre2_origem_receita_codigo_origem == 7
    "
  ...
  # Transferências de Capital (Origem 4)
...
if (!exists("dados_receita")) {
  ...
  # Aplicar critérios nas receitas da Seguridade Social
  ...
  datatable(
    ...
    caption = "📊 Receitas Orçamentárias - Seguridade Social",
...
if (exists("df_criterios_rreo_T01_receitas") && exists("df_criterios_rreo_T01_despesas_T01")) {
  ...
  cat(sprintf("RESULTADO:         R$ %s (%s)\n", 
              ...
              ifelse(resultado_seguridade >= 0, "SUPERÁVIT", "DÉFICIT")))
  cat(strrep("=", 60), "\n")
  
} else {
  resultado_seguridade <- NA
  cat("Dados não disponíveis para cálculo do resultado\n")
}
```

## 🥇 Seguridade Social por Área
...
| Objeto | Descrição |
|-------------------------------|-----------------------------------------|
| `criterios_rreo_T01_receitas` | Critérios para receitas orçamentárias |
| `criterios_rreo_T01_despesas_T01` | Critérios para despesas orçamentárias |
| `criterios_rreo_T01B_seguridade_T01B` | Critérios para seguridade por área |
| `df_criterios_rreo_T01_receitas` | Receitas processadas |
| `df_criterios_rreo_T01_despesas_T01` | Despesas processadas |
| `df_seguridade_area` | Despesas por área da seguridade |
| `resultado_seguridade` | Resultado (Receitas - Despesas) |

## 📌 Observações Importantes

1.  **Esfera Orçamentária**: Apenas Seguridade Social (código 2)

2.  **Receitas Intra-Orçamentárias**: Categorias 7 e 8 são incluídas

3.  **UOs Previdenciárias**:

    -   33904: INSS
    -   40904: FAT
    -   55902: RPPS
    -   25917: Ministério da Previdência

4.  **Modalidades de Transferência**: 30, 31, 35, 36, 40, 41, 42, 45

# 📋 RREO Tabela 02 - Despesas por Modalidade de Aplicação
...
| `elemento_despesa_codigo` | Elemento de despesa | 00-41 conforme classificação |
| `orgao_uge_orgao_maximo_codigo` | Órgão máximo | 52000 = Ministério da Defesa |
| `acao_governo_codigo` | Código da ação | 0181 = Pessoal Civil no MD |
| `orgao_uge_tipo_administracao_nome` | Tipo de administração | Direta, Autarquia, etc. |
| `orgao_uge_orcam_fiscal_s_n` | Indicador orçamento fiscal | PERTENCE |
| `mes_lancamento` | Mês de lançamento | Filtro do período |
...
-   Outras Despesas Variáveis (elemento 16)
-   Aposentadoria (elemento 01, exceto MD ou com ação 0181)
-   Pensões (elemento 03, exceto MD ou com ação 0181)
-   Contribuições a Entidades Fechadas de Previdência (elemento 07)
-   Obrigações Patronais (elemento 13)
-   Outras Aplicações

### Pessoal Militar

-   Vencimentos e Vantagens Fixas (elemento 12)
-   Outras Despesas Variáveis (elemento 17)
-   Reformas (elemento 01, MD, exceto ação 0181)
-   Pensões (elemento 03, MD, exceto ação 0181)
-   Obrigações Patronais (elemento 13, MD)
-   Outras Aplicações

### Outros

-   Transferências Intergovernamentais (elemento 41)
...
# ==============================================================================
# CRITÉRIOS DA TABELA 02 - DESPESAS COM PESSOAL POR MODALIDADE
# ==============================================================================
...
criterios_rreo_T02_despesas_T02 <- list(
  ...
  # PESSOAL CIVIL - OUTRAS DESPESAS VARIÁVEIS (Elemento 16)
  ...
  # Exclui MD (52000) ou inclui MD com ação 0181 (civis no MD)
  ...
  # PESSOAL CIVIL - PENSÕES (Elemento 03)
  # Exclui MD (52000) ou inclui MD com ação 0181 (civis no MD)
  ...
  # PESSOAL CIVIL - CONTRIBUIÇÕES A ENTIDADES FECHADAS DE PREVIDÊNCIA (Elemento 07)
  ...
  # PESSOAL CIVIL - OBRIGAÇÕES PATRONAIS (Elemento 13)
  ...
  # PESSOAL MILITAR - OUTRAS DESPESAS VARIÁVEIS (Elemento 17)
  `13  Pessoal Militar - Outras Despesas Variáveis` = list(
    criterio = "elemento_despesa_codigo %in% c('17')"
  ),
  
  # PESSOAL MILITAR - REFORMAS (Elemento 01, MD, exceto ação 0181)
  ...
  # PESSOAL MILITAR - PENSÕES (Elemento 03, MD, exceto ação 0181)
  ...
  # PESSOAL MILITAR - OBRIGAÇÕES PATRONAIS (Elemento 13, MD)
  ...
  # PESSOAL MILITAR - OUTRAS APLICAÇÕES (-)
  # Dedução para civis no MD (ação 0181)
  ...
  # TRANSFERÊNCIAS A ESTADOS E AO DF (Elemento 41)
...
### 📊 Despesa Liquidada por Tipo de Administração
...
if (exists("dados_despesa") && exists("mes_filtro")) {
  ...
  cat("Dados não disponíveis para validação\n")
}
```

### ✅ Validação Pessoal Civil
...
if (exists("validacao_civil") && exists("validacao_militar")) {
  ...
  cat("Dados não disponíveis para resumo consolidado\n")
}
```
...
| Objeto | Descrição |
|-------------------------------|-----------------------------------------|
| `criterios_rreo_T02_despesas_T02` | Critérios para despesas com pessoal |
| `df_t02_tipo_admin` | Despesas por tipo de administração |
| `validacao_civil` | Validação pessoal civil por tipo admin |
| `validacao_militar` | Validação pessoal militar por tipo admin |
...
1.  **Ministério da Defesa (52000)**:
    -   Pessoal militar está concentrado neste órgão
    -   Ação 0181 identifica civis lotados no MD
    -   Elementos 12 e 17 são exclusivos de militares
2.  **Elementos de Despesa**:
    -   01: Aposentadorias/Reformas
    -   03: Pensões
    -   07: Contribuições Previdência Fechada
    -   11: Vencimentos Fixos Civil
    -   12: Vencimentos Fixos Militar
    -   13: Obrigações Patronais
    -   16: Outras Variáveis Civil
    -   17: Outras Variáveis Militar
...
4.  **Filtros Aplicados**:
    ...
    -   Orçamento Fiscal (PERTENCE)
    -   Período de referência (mes_filtro)
    -   Item de informação 25 (Empenhado)

# 📋 RREO Tabela 03 - Despesas por Resultado EOF
...
|--------------------------|----------------------------------------------|
| **Relatório** | RREO - Relatório Resumido da Execução Orçamentária |
| **Demonstrativo** | Tabela 03 - Despesas por Resultado EOF (Primárias/Financeiras) |
| **Base Legal** | LRF Art. 52, inciso I |
| **Periodicidade** | Bimestral |
| **Grau de Dificuldade** | Fácil |
...
| Atributo | Descrição | Uso na Tabela |
|--------------------|----------------------|------------------------------|
| `resultado_eof_codigo` | Código do resultado EOF | 6 = Primário |
| `resultado_eof_nome` | Nome do resultado | Primário / Financeiro |
| `orgao_uge_orgao_maximo_codigo` | Código do órgão máximo | Agrupamento por órgão |
| `orgao_uge_orgao_maximo_nome` | Nome do órgão máximo | Descrição do órgão |
| `item_informacao_codigo` | Item de informação | 23 = Dotação Atualizada |
| `item_informacao_nome` | Nome do item | Descrição do item |
| `mes_lancamento` | Mês de lançamento | Filtro do período |
| `saldo_r_item_informacao` | Valor do saldo | Valor monetário |
...
| Anexo    | Vínculo                        |
|----------|--------------------------------|
| RREO A01 | Despesas orçamentárias totais  |
| RREO A06 | Resultado primário detalhado   |
| RGF A01  | Despesa com pessoal (primária) |

## 📐 Estrutura do Demonstrativo

### Classificação por Resultado EOF

**Resultado Primário (código 6):** - Despesas que afetam o resultado primário - Exclui despesas financeiras (juros, amortização) - Base para cálculo do resultado fiscal

**Resultado Financeiro:** - Despesas com juros e encargos da dívida - Despesas com amortização da dívida - Operações financeiras diversas

### Itens de Informação Típicos

-   **23**: Dotação Atualizada
...
if (!exists("dados_despesa")) {
  ...
  # Formatar e exibir
  cat(sprintf("💰 Total das Despesas Primárias (Dotação Atualizada): R$ %s\n",
...
### Despesas Primárias por Item de Informação
...
# DESPESAS PRIMÁRIAS POR ITEM DE INFORMAÇÃO
# ==============================================================================
# Compara diferentes estágios da despesa
# ==============================================================================

if (exists("dados_despesa")) {
  # Agrupar por item de informação
  ...
  datatable(
    ...
    caption = "📊 Despesas Primárias por Item de Informação",
...
### Despesas Primárias por Órgão e Item de Informação
...
# ==============================================================================
# DESPESAS PRIMÁRIAS DETALHADAS POR ÓRGÃO E ITEM
# ==============================================================================

if (exists("dados_despesa")) {
  # Agrupar por órgão e item
  ...
  datatable(
    ...
    caption = "📊 Despesas Primárias por Órgão e Estágio",
...
if (exists("despesas_por_resultado") && exists("total_despesas_primarias")) {
  ...
  cat(strrep("=", 70), "\n")
  cat(sprintf("Total de Despesas Primárias:    R$ %s\n",
              format(round(total_despesas_primarias, 2), big.mark = ".", decimal.mark = ",")))
  cat(sprintf("Total Geral de Despesas:        R$ %s\n",
              format(round(total_geral, 2), big.mark = ".", decimal.mark = ",")))
  cat(strrep("-", 70), "\n")
  cat(sprintf("Percentual Primário:            %.2f%%\n", percentual_primario))
  cat(strrep("=", 70), "\n")
...
| Objeto | Descrição |
|-------------------------------|-----------------------------------------|
| `agrupado_despesa_orgao` | Vetor de agrupamento por órgão |
| `total_despesas_primarias` | Total das despesas primárias |
| `despesas_por_resultado` | Despesas por resultado EOF |
| `despesas_primarias_orgao` | Despesas primárias por órgão |
| `despesas_primarias_item` | Despesas primárias por item de informação |
| `despesas_primarias_detalhado` | Despesas por órgão e estágio |
| `valor_despesas_primarias_t03` | Total primário para exportação |
| `valor_despesas_financeiras_t03` | Total financeiro para exportação |

## 📌 Observações Importantes

1.  **Resultado EOF (Esfera Orçamentário-Financeira)**:
    -   Código 6 = Despesas Primárias
    -   Demais códigos = Despesas Financeiras
2.  **Despesas Primárias**:
    -   Afetam o resultado primário do governo
    -   Excluem juros, encargos e amortização da dívida
    -   Base para o cálculo do superávit/déficit primário
3.  **Despesas Financeiras**:
    -   Juros e encargos da dívida
    -   Amortização da dívida
    -   Operações financeiras
4.  **Itens de Informação**:
    -   23: Dotação Atualizada (LOA + Créditos)
    -   24: Despesa Empenhada
    -   25: Despesa Liquidada
    -   26: Despesa Paga
5.  **Relação com Resultado Primário**:
    -   As despesas primárias são componente do resultado primário
    -   O Anexo 06 do RREO detalha o cálculo completo
...
# ==============================================================================
# DIAGNÓSTICO: Investigar Objetos Vazios
# ==============================================================================

# Criar consolidação temporária para diagnóstico
dados_consolidados_temp <- consolidar_criterios(save_global = FALSE)

cat("\n📊 ESTRUTURA COMPLETA DOS DADOS CONSOLIDADOS:\n\n")

# Ver distribuição por relatório e anexo
...
if (nrow(rgf_a01_todos) == 0) {
  cat("❌ Nenhum dado encontrado para RGF A01\n")
  cat("Verificando possíveis variações:\n")
  ...
  cat(sprintf("✅ Encontrados %d linhas em RGF A01\n", nrow(rgf_a01_todos)))
  cat("\nValores únicos:\n")
...
if (nrow(rgf_a03_todos) == 0) {
  cat("❌ Nenhum dado encontrado para RGF A03\n")
} else {
  cat(sprintf("✅ Encontrados %d linhas em RGF A03\n", nrow(rgf_a03_todos)))
}
...
# ==============================================================================
# 4. INVESTIGAR RREO A01 - Balanço Orçamentário (Receitas/Despesas)
# ==============================================================================
...
# ==============================================================================
# 5. RESUMO DE TODOS OS CÓDIGOS DISPONÍVEIS
# ==============================================================================
...
# PARTE FINAL - VALIDAÇÕES

# Resumo Executivo

Este chunk apresenta um resumo rápido dos principais indicadores calculados.
...
# Função auxiliar para formatar valores em bilhões (com tratamento robusto)
fmt_bi <- function(x) {
  tryCatch({
    x_num <- as.numeric(x)
...
# Função para extrair valor com segurança
...
# DCL - tentar múltiplas variáveis
v_dcl <- safe_extract(
...
v_res_prim <- safe_extract(if (exists("resultado_primario") && is.data.frame(resultado_primario)) {
  resultado_primario %>% filter(grepl("RESULTADO PRIMÁRIO|RESULTADO PRIMARIO", componente)) %>% pull(valor) %>% first()
...
# Variáveis calculadas no chunk rgf-a05: valor_disponibilidade_executivo e valor_disponibilidade_uniao
# - Meses normais: disponibilidade_antes_inscr_rp_clean
# - Dezembro: disponibilidade após inscrição de RP (ajustar no chunk original se necessário)
...
v_disp_executivo <- safe_extract({
  if (exists("valor_disponibilidade_executivo") && length(valor_disponibilidade_executivo) > 0) {
    ...
    # A linha de total pode ser "Total", "-" ou a última linha
    ...
    if (nrow(linha_total) > 0) {
      ...
      # Pegar última linha (total adicionado por adorn_totals)
...
status_regra <- if (exists("status_regra_ouro")) status_regra_ouro else 
                if (!is.na(v_regra_ouro)) ifelse(v_regra_ouro > 0, "NÃO CUMPRIDA", "CUMPRIDA") else "-"

# MDE e ASPS
v_mde_pct <- safe_extract(if (exists("percentual_aplicacao")) as.numeric(percentual_aplicacao) else NA_real_)

# ASPS - tentar múltiplas variáveis (pct_cumprimento é calculado no chunk de saúde)
v_asps_pct <- safe_extract({
  ...
  if (exists("pct_cumprimento") && !is.null(pct_cumprimento) && length(pct_cumprimento) > 0) {
    ...
    # Se o valor vier muito alto (>1000), provavelmente já está multiplicado por 100 ou 1000
    ...
    if (!is.na(ms) && ms != 0) {
      # total_despesas_computadas já está em R$, minimo_saude também
...
indicadores_fiscais <- tibble(
  ...
  Status = c(
    ...
    if (!is.na(v_res_prim)) ifelse(v_res_prim >= 0, "✅ Superávit", "⚠️ Déficit") else "-",
    if (!is.na(v_res_nominal)) ifelse(v_res_nominal >= 0, "✅ Superávit", "⚠️ Déficit") else "-"
  )
)
...
# ==============================================================================
# TABELA 2: PREVIDÊNCIA - RESULTADOS POR REGIME
# ==============================================================================
...
indicadores_prev <- tibble(
  ...
  Status = c(
    if (!is.na(v_rgps)) ifelse(v_rgps >= 0, "✅ Superávit", "⚠️ Déficit") else "-",
    if (!is.na(v_rpps_civil)) ifelse(v_rpps_civil >= 0, "✅ Superávit", "⚠️ Déficit") else "-",
    if (!is.na(v_rpps_militar)) ifelse(v_rpps_militar >= 0, "✅ Superávit", "⚠️ Déficit") else "-",
    if (!is.na(v_fcdf)) ifelse(v_fcdf >= 0, "✅ Superávit", "⚠️ Déficit") else "-"
  )
)

cat("\n")
cat("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
cat("                    PREVIDÊNCIA - RESULTADOS POR REGIME\n")
cat("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
...
indicadores_outros <- tibble(
  Indicador = c(
    "📋 Tabela 3 - Despesas Primárias",
    "📋 Tabela 1 - Resultado Seguridade",
    "📋 Regra de Ouro",
    "📋 Disponibilidade Caixa - Executivo",
    "📋 Disponibilidade Caixa - União"
  ),
  `Valor (R$ bi)` = c(
    fmt_bi(v_tabela3),
    fmt_bi(v_seguridade),
  ...
  Status = c(
    "-",
    if (!is.na(v_seguridade)) ifelse(v_seguridade >= 0, "✅ Superávit", "⚠️ Déficit") else "-",
    status_regra,
    "-",
    "-"
  )
...
# ==============================================================================
# TABELA 5: APLICAÇÕES CONSTITUCIONAIS
# ==============================================================================
...
indicadores_const <- tibble(
  ...
  Status = c(
    if (!is.na(v_mde_pct) && v_mde_pct >= 100) "✅ Atende" else "⚠️ Verificar",
    if (!is.na(v_asps_pct) && v_asps_pct >= 100) "✅ Atende" else "⚠️ Verificar"
  )
)

cat("\n")
cat("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
cat("                    APLICAÇÕES CONSTITUCIONAIS\n")
cat("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
...
Comparação dos valores calculados pelo sistema com o RREO Anexo 14 oficial.
...
# Ler célula do Excel com segurança
...
# Extrair valor de dataframe com segurança
...
# Exibir tabela de validação formatada
...
# ==============================================================================
# MAPEAMENTO DAS CÉLULAS DO RREO ANEXO 14 SIMPLIFICADO
# ==============================================================================

mapa_rreo_a14 <- tribble(
  ~item, ~celula, ~descricao, ~fonte,
  
  # Balanço Orçamentário - Receitas (Anexo 1)
  "bo_previsao_inicial",    "B10", "BO - Previsão Inicial Receita", "A01",
  "bo_previsao_atualizada", "B11", "BO - Previsão Atualizada", "A01",
  "bo_receitas_realizadas", "B12", "BO - Receitas Realizadas", "A01",
  "bo_saldos_anteriores",   "B14", "BO - Saldos Exerc. Anteriores", "A01",
  
  # Balanço Orçamentário - Despesas (Anexo 2)
  "bo_dotacao_inicial",     "B17", "BO - Dotação Inicial", "A02",
  "bo_dotacao_atualizada",  "B18", "BO - Dotação Atualizada", "A02",
  ...
  "bo_superavit",           "B22", "BO - Superávit Orçamentário", "A02",
  
  # Despesas por Função (exceto refinanciamento)
  "funcao_empenhadas",      "B25", "Função - Desp. Empenhadas", "A02",
  "funcao_liquidadas",      "B26", "Função - Desp. Liquidadas", "A02",
  
  # RCL (Anexo 3)
  "rcl",                    "B29", "Receita Corrente Líquida", "A03",
  ...
  "resultado_primario",     "B59", "Resultado Primário", "A06",
  
  # Restos a Pagar Processados (Anexo 7)
  "rpp_inscricao",          "B62", "RP Proc - Inscrição", "A07",
  "rpp_cancelamento",       "C62", "RP Proc - Cancelamento", "A07",
  "rpp_pagamento",          "D62", "RP Proc - Pagamento", "A07",
  "rpp_saldo",              "E62", "RP Proc - Saldo", "A07",
  
  # Restos a Pagar Não Processados (Anexo 7)
  "rpnp_inscricao",         "B68", "RPNP - Inscrição", "A07",
  ...
  "mde_minimo",             "C77", "MDE - Mínimo a Aplicar", "A08",
  "fundeb_complementacao",  "B79", "Complementação FUNDEB", "A08",
  
  # ASPS (Anexo 12)
  "asps_aplicado",          "B82", "ASPS - Valor Aplicado", "A12",
  "asps_minimo",            "C82", "ASPS - Mínimo a Aplicar", "A12"
)
```
...
# Resultado Primário e Nominal (fonte: rp_claude.xlsm)
...
# ==============================================================================
# COMPARAÇÃO RREO: SISTEMA VS OFICIAL
# ==============================================================================
...
if (file.exists(arquivo_rreo_a14)) {
  ...
  # Exibir
  exibir_validacao(comparacao_rreo, "VALIDAÇÃO RREO ANEXO 14 - Sistema vs Oficial (R$ milhares)")
  
} else {
  cat(sprintf("⚠️ Arquivo não encontrado: %s\n", arquivo_rreo_a14))
}
```

## Validação RGF - Código Completo Corrigido

Substitua os chunks de validação RGF pelos códigos abaixo.

---

### Chunk 1: Mapeamento de Células do Excel Oficial

```{r mapeamento-rgf}
#| echo: false

# ==============================================================================
# MAPEAMENTO DAS CÉLULAS DO EXCEL OFICIAL - RGF
# ==============================================================================

mapeamento_rgf <- tribble(
  ~item,                        ~celula, ~descricao,                          ~fonte,
  "rcl",                        "B10",   "Receita Corrente Líquida",          "RGF",
  "dtp_executivo",              "B15",   "DTP - Poder Executivo",             "A01",
  "dtp_amapa",                  "B20",   "DTP - Amapá",                       "A01",
  "dtp_roraima",                "B25",   "DTP - Roraima",                     "A01",
  "dtp_df",                     "B30",   "DTP - Distrito Federal",            "A01",
  "dcl",                        "B36",   "Dívida Consolidada Líquida",        "A02",
  "garantias",                  "B40",   "Total das Garantias",               "A03",
  "op_credito",                 "B44",   "Operações de Crédito",              "A04",
  "rpnp_exercicio",             "B52",   "RPNP do Exercício",                 "A05",
  "disponibilidade_antes_rp",   "C52",   "Disp. Caixa ANTES Inscrição RP",    "A05"
)

cat("✅ Mapeamento RGF definido:", nrow(mapeamento_rgf), "itens\n")
```
...
# Extraído do dataframe "pessoal" gerado no RGF A01
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# DCL - Dívida Consolidada Líquida (RGF A02)
# Usar valor_dcl_num (numérico)
# -----------------------------------------------------------------------------
...
# RPNP do Exercício (só tem valor em dezembro)
...
# Disponibilidade ANTES da inscrição em RP
...
# Disponibilidade APÓS a inscrição em RP (só tem valor em dezembro)
...
cat("\n✅ DataFrame sistema_rgf criado com", nrow(sistema_rgf), "linhas\n")
```
...
# Buscar arquivo pelo padrão "Anexo 6 Simplificado"
...
if (length(arquivos_encontrados) > 0) {
  ...
  cat("📁 Arquivo encontrado:", basename(arquivo_comparativo), "\n")
  ...
  # Função para ler célula - CORRIGIDA
  ler_celula <- function(celula) {
    ...
    # Converter letra para número (A=1, B=2, etc.)
    ...
    # Se é texto, converter formato brasileiro para numérico
    if (is.character(valor)) {
      # Remover espaços
      valor <- trimws(valor)
      
      # Verificar se tem vírgula como decimal (formato BR: 1.234.567,89)
      if (grepl(",", valor) && grepl("\\.", valor)) {
        ...
        valor <- gsub(",", ".", valor)   # Troca vírgula decimal por ponto
      } else if (grepl(",", valor)) {
        # Só tem vírgula (decimal)
        valor <- gsub(",", ".", valor)
      }
      # Se só tem pontos, assume que são milhares e o número é inteiro
      # (não faz nada, deixa o R interpretar)
    }
  ...
  cat("\n✅ Valores oficiais RGF lidos:", sum(!is.na(oficial_rgf$valor_oficial)), "de", nrow(oficial_rgf), "\n")
  ...
  cat("⚠️ Arquivo 'Anexo 6 Simplificado' não encontrado em:", mes_pasta, "\n")
}
...
### Chunk 4: Comparação Sistema vs Oficial

```{r comparacao-rgf}
#| echo: false

# ==============================================================================
# COMPARAÇÃO SISTEMA vs OFICIAL - RGF
# ==============================================================================
...
comparacao_rgf <- oficial_rgf %>%
  ...
  mutate(
    # Converter oficial para mesma escala (se necessário)
...
cat("✅ Comparação RGF concluída\n")
```

---

### Chunk 5: Exibir Tabela de Validação
...
datatable(
  tabela_rgf,
  caption = "Validação RGF - Sistema vs Oficial",
  filter = "top",
  options = list(
    pageLength = 15,
    scrollX = TRUE,
  ...
  formatStyle(
    ...
    backgroundColor = styleEqual(
      c("✅ OK", "⚠️ Verificar", "❌ DIVERGENTE", "Sem sistema"),
...
### Chunk 6: Resumo da Validação

```{r resumo-validacao-rgf}
#| echo: false

# ==============================================================================
# RESUMO DA VALIDAÇÃO - RGF
# ==============================================================================
...
1. **DTP (Despesa Total com Pessoal)**: Agora é extraído corretamente do dataframe `pessoal` filtrado por `categoria_pessoal`

2. **Disponibilidades**: 
   - `disponibilidade_antes_rp`: Disponibilidade antes da inscrição em RP (todos os meses)
   - `disponibilidade_apos_rp`: Disponibilidade após a inscrição em RP (só dezembro)

3. **RPNP do Exercício**: Só terá valor em dezembro

4. **Limites removidos**: A tabela agora mostra apenas valores realizados, sem os limites calculados

5. **Células do Excel**: Ajustar conforme layout do arquivo `comparativo.xlsx`


## 📌 Observações

1.  **Status da Validação**:
    -   ✅ **OK**: Sem diferença (\< 0,5 milhar ou \< 0,1%)
    -   ⚠️ **Verificar**: Diferença entre 0,1% e 1%
    -   ❌ **DIVERGENTE**: Diferença \> 1%
    -   ⬜ **Sem oficial/sistema**: Valor não disponível
2.  **Arquivos Oficiais Necessários**:
    -   `Anexo 14 - Simplificado.xlsm` (RREO)
    -   `Anexo 6 Simplificado - 2025.xlsx` (RGF)
3.  **Fontes de Dados Especiais**:
    -   Resultado Primário/Nominal: `rp_claude.xlsm` (planilha do setor)
    -   Valores em R\$ milhares para comparação com relatórios oficiais

# 📊 Mapeamento de Atributos por Demonstrativo

Este capítulo apresenta uma análise automática de todos os atributos (colunas) utilizados nos critérios de classificação dos demonstrativos fiscais.
...
#| echo: true
#| code-fold: true
#| code-summary: "Ver código de extração de atributos"
...
#| echo: true
#| code-fold: true
#| code-summary: "Ver coleta de todos os critérios"

# ==============================================================================
# COLETA DE ATRIBUTOS DE TODOS OS CRITÉRIOS DEFINIDOS
# ==============================================================================

cat("📊 Coletando atributos de todos os critérios...\n\n")
...
atributos_coletados <- bind_rows(
  ...
  # RREO A01 - Balanço Orçamentário
  processar_se_existe("criterios_rreo_A01_receitas_bo", "RREO", "A01 - Balanço Orçamentário"),
  processar_se_existe("criterios_rreo_A01_despesas_bo", "RREO", "A01 - Balanço Orçamentário"),
  ...
  processar_se_existe("criterios_rreo_A06_receitas_A06", "RREO", "A06 - Result. Primário"),
  processar_se_existe("criterios_rreo_A06_despesas_categoria_grupo_A06", "RREO", "A06 - Result. Primário"),
  processar_se_existe("criterios_rreo_A06_despesas_natureza_detalhada_A06", "RREO", "A06 - Result. Primário"),
  processar_se_existe("criterios_rreo_A06_disponibilidades_A06", "RREO", "A06 - Result. Primário"),
  processar_se_existe("criterios_rreo_A06_juros_ativos_passivos_A06", "RREO", "A06 - Result. Primário"),
  ...
  processar_se_existe("criterios_rreo_T01B_seguridade_T01B", "RREO", "T01B - Seguridade Área"),
  
  # RREO T02 - Despesas com Pessoal
  processar_se_existe("criterios_rreo_T02_despesas_T02", "RREO", "T02 - Pessoal")
)

cat(sprintf("✅ Total de registros coletados: %d\n", nrow(atributos_coletados)))
cat(sprintf("✅ Atributos únicos identificados: %d\n", n_distinct(atributos_coletados$atributo)))
```

## 📋 Resumo por Demonstrativo e Anexo
...
if (nrow(atributos_coletados) > 0) {
  ...
  datatable(
    ...
    colnames = c("Demonstrativo", "Anexo", "Atributo", "Qtd Usos", "Critérios"),
    rownames = FALSE
  )
  
} else {
  cat("⚠️ Nenhum atributo coletado. Verifique se os critérios estão definidos.\n")
}
```

## 📊 Atributos Mais Utilizados (Geral)
...
if (nrow(atributos_coletados) > 0) {
  ...
  cat("⚠️ Nenhum atributo coletado.\n")
}
```

## 📈 Estatísticas por Demonstrativo

```{r estatisticas-demonstrativo}
#| echo: false

# ==============================================================================
# ESTATÍSTICAS AGREGADAS POR DEMONSTRATIVO
# ==============================================================================
...
if (nrow(atributos_coletados) > 0) {
  ...
  knitr::kable(
    stats_demonstrativo,
    col.names = c("Demonstrativo", "Anexos", "Critérios", "Atributos Únicos", "Total Usos"),
    caption = "Estatísticas por Demonstrativo"
  )
  
}
```

## 🔍 Matriz de Atributos x Anexos
...
# ==============================================================================
# MATRIZ: QUAIS ATRIBUTOS SÃO USADOS EM QUAIS ANEXOS
# ==============================================================================
...
if (nrow(atributos_coletados) > 0) {
  ...
  matriz_atributos <- matriz_atributos %>%
    rowwise() %>%
    mutate(
      total_anexos = sum(c_across(-atributo) == "✓")
    ) %>%
    ungroup() %>%
  ...
  datatable(
    matriz_atributos,
    caption = "Matriz de Atributos x Anexos (✓ = atributo utilizado)",
    filter = "top",
    extensions = "Buttons",
    options = list(
      dom = "Bfrtip",
...
# ==============================================================================
# LISTA COMPLETA DE CRITÉRIOS POR ANEXO
# ==============================================================================
...
if (nrow(atributos_coletados) > 0) {
  ...
  datatable(
    lista_criterios,
    caption = "Lista Completa de Critérios e seus Atributos",
    filter = "top",
    extensions = "Buttons",
    options = list(
      dom = "Bfrtip",
    ...
    colnames = c("Demonstrativo", "Anexo", "Critério", "Atributos Utilizados", "Qtd"),
    rownames = FALSE
  )
...
# ==============================================================================
# EXPORTAR MAPEAMENTO PARA ANÁLISE EXTERNA
# ==============================================================================
...
if (nrow(atributos_coletados) > 0) {
  ...
  cat("✅ Objetos de mapeamento disponíveis:\n")
...
1.  **Identificação de padrões**: O código busca padrões como `atributo %in% c(...)`, `atributo == valor`, `startsWith(atributo, ...)`, etc.

2.  **Limpeza**: Remove palavras reservadas (TRUE, FALSE, NA) e matches muito curtos.

3.  **Agregação**: Agrupa por demonstrativo/anexo e conta ocorrências.
:::

::: callout-warning
### Limitações

-   Atributos usados em filtros fora das listas de critérios (ex: filtros diretos em `filter()`) não são capturados automaticamente.
-   Nomes de colunas muito curtos (≤2 caracteres) são ignorados para evitar falsos positivos.
:::
