...
# Total de Disponibilidade - União
...
|--------|---------|-----------|
| `dados_despesa` | tg_despesa_*.xlsx | Despesas orçamentárias com classificação funcional |
...
| Objeto | Arquivo | Descrição |
|--------|---------|-----------|
| `dados_receita` | tg_receita_*.xlsx | Receitas orçamentárias |
...
| `dados_despesa` | tg_despesa_*.xlsx | Despesas orçamentárias (subsídios CESEF) |
...
**Por Situação:**
- **RP Processados**: Despesas empenhadas e liquidadas, pendentes de pagamento
- **RP Não Processados**: Despesas empenhadas mas não liquidadas

**Por Modalidade:**
- **Intragovernamental**: Operações entre órgãos do mesmo ente
- **Exceto Intragovernamental**: Demais operações

**Movimentação:**
- Inscritos em Exercícios Anteriores
- Inscritos em 31/12 do Exercício Anterior
- Pagos
- Cancelados
- Saldo
...
| Objeto | Arquivo | Descrição |
|--------|---------|-----------|
| `dados_receita` | tg_receita_*.xlsx | Receitas orçamentárias (impostos) |
| `dados_despesa` | tg_despesa_*.xlsx | Despesas orçamentárias com educação |
...
if (exists("dados_despesa") && exists("mes_filtro")) {
  despesa_mde <- aplicar_criterios(
    dados_despesa %>% filter(item_informacao_codigo == 25), 
...
| Objeto | Arquivo | Descrição |
|--------|---------|-----------|
| `dados_receita` | tg_receita_*.xlsx | Receitas de operações de crédito |
| `dados_despesa` | tg_despesa_*.xlsx | Despesas de capital |
...
|--------|---------|-----------|
| `dados_receita` | tg_receita_*.xlsx | Receitas orçamentárias |
| `dados_despesa` | tg_despesa_*.xlsx | Despesas orçamentárias |
...
if (!exists("dados_despesa")) {
  ...
  df_criterios_rreo_T01_despesas_T01 <- aplicar_criterios(
    df = dados_despesa %>% 
      filter(esfera_orcamentaria_codigo == 2, item_informacao_codigo == "25") %>% 
      unique(),
...
**Resultado Primário (código 6):**
- Despesas que afetam o resultado primário
- Exclui despesas financeiras (juros, amortização)
- Base para cálculo do resultado fiscal
...
Comparação dos valores calculados pelo sistema com o RGF Anexo 06 oficial.

## Mapeamento RGF Anexo 06
...
# ==============================================================================
# MAPEAMENTO DAS CÉLULAS DO RGF ANEXO 06 SIMPLIFICADO
# ==============================================================================
...
mapa_rgf_a06 <- tribble(
  ...
  "limite_max_exec",       "B16", "Limite Máximo - Executivo", "A01",
  ...
  "limite_max_ap",         "B21", "Limite Máximo - Amapá", "A01",
  
  # Pessoal - Roraima
  "dtp_roraima",           "B25", "DTP - Roraima", "A01",
  "limite_max_rr",         "B26", "Limite Máximo - Roraima", "A01",
  
  # Pessoal - DF
  "dtp_df",                "B30", "DTP - Distrito Federal", "A01",
  "limite_max_df",         "B31", "Limite Máximo - DF", "A01",
  
  # Dívida (Anexo 2)
  "dcl",                   "B36", "Dívida Consolidada Líquida", "A02",
  "limite_dcl",            "B37", "Limite DCL (Resolução SF)", "A02",
  ...
  "limite_op_credito",     "B45", "Limite Op. Crédito", "A04",
  
  # Disponibilidade (Anexo 5)
  "rpnp_exercicio",        "B52", "RPNP do Exercício", "A05",
  "disponibilidade_caixa", "C52", "Disponibilidade Caixa Líquida", "A05"
)
```

## Coleta de Valores do Sistema - RGF

```{r valores-sistema-rgf}
#| echo: true
#| label: valores-sistema-rgf

# ==============================================================================
# COLETA DOS VALORES CALCULADOS PELO SISTEMA - RGF
# ==============================================================================
...
# Despesa com Pessoal por Ente
valor_dtp_exec <- if (exists("dtp_executivo")) safe_num(dtp_executivo) else NA_real_
valor_dtp_ap <- if (exists("dtp_amapa")) safe_num(dtp_amapa) else NA_real_
valor_dtp_rr <- if (exists("dtp_roraima")) safe_num(dtp_roraima) else NA_real_
valor_dtp_df <- if (exists("dtp_df")) safe_num(dtp_df) else NA_real_

# DCL
valor_dcl_rgf <- if (exists("valor_dcl")) safe_num(valor_dcl) else NA_real_

# Garantias
valor_garantias_rgf <- if (exists("total_garantias")) safe_num(total_garantias) else NA_real_

# Operações de Crédito
valor_op_cred <- if (exists("op_credito_total")) safe_num(op_credito_total) else NA_real_

# Disponibilidade (Anexo 5)
valor_rpnp_ex <- if (exists("rpnp_exercicio")) safe_num(rpnp_exercicio) else NA_real_
valor_disp_caixa <- if (exists("disponibilidade_caixa_liquida")) safe_num(disponibilidade_caixa_liquida) else NA_real_

# Calcular limites a partir da RCL (com proteção)
rcl_rgf_milhar <- if (!is.na(valor_rcl_rgf)) valor_rcl_rgf / 1000 else NA_real_
limite_max_exec <- if (!is.na(rcl_rgf_milhar)) rcl_rgf_milhar * 0.379 else NA_real_
limite_prud_exec <- if (!is.na(limite_max_exec)) limite_max_exec * 0.95 else NA_real_
limite_alerta_exec <- if (!is.na(limite_max_exec)) limite_max_exec * 0.90 else NA_real_

# Montar dataframe do sistema (todos valores numéricos)
sistema_rgf <- tibble(
  item = c(
    "rcl",
    "dtp_executivo", "limite_max_exec", "limite_prud_exec", "limite_alerta_exec",
    "dtp_amapa", "limite_max_ap",
    "dtp_roraima", "limite_max_rr",
    "dtp_df", "limite_max_df",
    "dcl", "limite_dcl",
  ...
  valor = as.numeric(c(
    valor_rcl_rgf,
    valor_dtp_exec, 
    if (!is.na(limite_max_exec)) limite_max_exec * 1000 else NA_real_, 
    if (!is.na(limite_prud_exec)) limite_prud_exec * 1000 else NA_real_, 
    if (!is.na(limite_alerta_exec)) limite_alerta_exec * 1000 else NA_real_,
    valor_dtp_ap, NA_real_,
    valor_dtp_rr, NA_real_,
    valor_dtp_df, NA_real_,
    valor_dcl_rgf, NA_real_,
    valor_garantias_rgf, NA_real_,
    valor_op_cred, NA_real_,
    valor_rpnp_ex, valor_disp_caixa
  ))
) %>%
  mutate(valor_milhar = valor / 1000)
```
...
if (file.exists(arquivo_rgf_a06)) {
  ...
  # Comparar
  comparacao_rgf <- comparar_valores(rgf_oficial, sistema_rgf)
  
  # Exibir
  exibir_validacao(comparacao_rgf, "VALIDAÇÃO RGF ANEXO 06 - Sistema vs Oficial (R$ milhares)")
  
} else {
  cat(sprintf("⚠️ Arquivo não encontrado: %s\n", arquivo_rgf_a06))
  cat("Verifique se o arquivo 'Anexo 6 Simplificado - 2025.xlsx' está na pasta de dados.\n")
}
```
...
```{r sumario-validacao}
#| echo: false
#| label: sumario-validacao

# ==============================================================================
# SUMÁRIO CONSOLIDADO DAS VALIDAÇÕES
# ==============================================================================

cat("\n")
cat(strrep("=", 70), "\n")
cat("SUMÁRIO DE VALIDAÇÃO\n")
cat(strrep("=", 70), "\n")
cat(sprintf("Período: %s\n", mes_filtro))
cat(strrep("-", 70), "\n")

# RREO
if (exists("comparacao_rreo")) {
  resumo_rreo <- comparacao_rreo %>% count(status)
  cat("\n📊 RREO ANEXO 14:\n")
  for (i in 1:nrow(resumo_rreo)) {
    emoji <- case_when(
      resumo_rreo$status[i] == "OK" ~ "✅",
      resumo_rreo$status[i] == "Verificar" ~ "⚠️",
      resumo_rreo$status[i] == "DIVERGENTE" ~ "❌",
      TRUE ~ "⬜"
    )
    cat(sprintf("   %s %s: %d itens\n", emoji, resumo_rreo$status[i], resumo_rreo$n[i]))
  }
}

# RGF
if (exists("comparacao_rgf")) {
  resumo_rgf <- comparacao_rgf %>% count(status)
  cat("\n📊 RGF ANEXO 06:\n")
  for (i in 1:nrow(resumo_rgf)) {
    emoji <- case_when(
      resumo_rgf$status[i] == "OK" ~ "✅",
      resumo_rgf$status[i] == "Verificar" ~ "⚠️",
      resumo_rgf$status[i] == "DIVERGENTE" ~ "❌",
      TRUE ~ "⬜"
    )
    cat(sprintf("   %s %s: %d itens\n", emoji, resumo_rgf$status[i], resumo_rgf$n[i]))
  }
}

cat(strrep("=", 70), "\n")

# Exportar objetos para uso posterior
validacao_rreo <- if (exists("comparacao_rreo")) comparacao_rreo else NULL
validacao_rgf <- if (exists("comparacao_rgf")) comparacao_rgf else NULL
```
...
| Objeto | Descrição |
|--------|-----------|
| `mapa_rreo_a14` | Mapeamento de células do RREO Anexo 14 |
| `mapa_rgf_a06` | Mapeamento de células do RGF Anexo 06 |
| `sistema_rreo` | Valores calculados pelo sistema (RREO) |
| `sistema_rgf` | Valores calculados pelo sistema (RGF) |
| `comparacao_rreo` | Resultado da validação RREO |
| `comparacao_rgf` | Resultado da validação RGF |
| `validacao_rreo` | Alias para comparacao_rreo |
| `validacao_rgf` | Alias para comparacao_rgf |
