...
```{r}
# =============================================================================
# CÁLCULO DA DCL A PARTIR DE valores_linhas
# =============================================================================

calcular_dcl_simples <- function(df) {
  
  # Função auxiliar para pegar valor por ordem
  v <- function(ord) {
    val <- df$valor_total[df$ordem == ord]
    if (length(val) == 0) 0 else val
  }
  
  # DÍVIDA CONSOLIDADA (DC)
  divida_mobiliaria <- v("01") - v("02") + v("03") + v("04") + v("05") - v("26")
  DC <- divida_mobiliaria + v("06") + v("07") + v("08") + v("09") + v("10") - v("38")
  
  # DEDUÇÕES
  ativo_disponivel <- v("11") + v("12")
  aplicacoes_fin <- - v("13") - v("14") + v("15") + v("16")  # FAT + Fundos Setor Privado
  renegociacao <-  v("20") - v("21")
  demais_ativos <- v("22") - v("23")
  haveres_fin <- aplicacoes_fin + renegociacao + demais_ativos
  
  DEDUCOES <- ativo_disponivel + haveres_fin - v("25")  # RP subtrai
  
  # DCL
  DCL <- DC - DEDUCOES
  
  # Retorno em R$ milhares
  list(DC = DC/1000, DEDUCOES = DEDUCOES/1000, DCL = DCL/1000)
}

# Executar
resultado <- calcular_dcl_simples(valores_linhas)

cat("DC:", format(resultado$DC, big.mark = ".", decimal.mark = ","), "mil\n")
cat("Deduções:", format(resultado$DEDUCOES, big.mark = ".", decimal.mark = ","), "mil\n")
cat("DCL:", format(resultado$DCL, big.mark = ".", decimal.mark = ","), "mil\n")
```
