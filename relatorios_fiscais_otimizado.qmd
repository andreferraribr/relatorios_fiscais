...
1. ✅ Importa os dados
2. ✅ Converte `movim_liquido_r_item_informacao` para numérico
3. 🔑 **CRIA `saldo_r_item_informacao = movim_liquido_r_item_informacao`** (alias)
4. ✅ Verifica que ambas as colunas existem e são numéricas
5. ✅ Testa agregação em ambas (devem dar o mesmo resultado)

---

## 📊 **RESULTADO ESPERADO**
```
📥 Importando dados RCL (RREO A03)...
   • Importando RCL Despesas...
   • Importando RCL Receitas...

✓ RCL Despesas: 26.346 registros
✓ RCL Receitas: 21.719 registros

🔍 Verificação:
   Colunas em despesas: 12
   Colunas em receitas: 17

✅ Alias 'saldo_r_item_informacao' criado:
   Despesas: TRUE (tipo: numeric)
   Receitas: TRUE (tipo: numeric)

🧪 Teste de agregação:
   Despesas (movim_liquido): 123.456.789.012,34
   Despesas (saldo_r alias): 123.456.789.012,34
   Receitas (movim_liquido): 98.765.432.109,87
   Receitas (saldo_r alias): 98.765.432.109,87
...
# Extrair ano e mês do mes_filtro
...
apuracao <- rcl %>%
  filter(
    year(data_lancamento) == ano_filtro,
    month(data_lancamento) <= mes_ref
  )  %>%
  mutate(
    tipo =
      case_when(
...
#   filter(data_lancamento >= max(data_lancamento) %m-% months(10)) %>% 
#   mutate(
#     tipo =
#       case_when(
#         ordem %in% c("01" , "02", "03", "04", "05", "06", "07", "08" , "09") ~ "receitas",
